...
export default class Options {
    ...
    get docsify () {
        ...
        if ( Object.isEmpty( options.types ) ) delete options.types;
        if ( Object.isEmpty( options.alias ) ) delete options.alias;
        ...
        if ( !this.#types ) {
            ...
            if ( this.#config.externalTypes ) {
                for ( let url of this.#config.externalTypes ) {
                    url = new URL( ".docs.config.yaml", url );
                    ...
                    const config = YAML.loadAll( await res.text() )[0];
                    ...
                    for ( const type in config.types ) {
                        ...
                        this.#types[type] = url.href.replaceAll( "/.docs.config.yaml", "/" ) + "#" + this.#buildTypeURL( this.#types[type] );
                    }
                }
            }

            this.#types = { ...this.#types, ...DEFAULT_TYPES, ...( this.#config.types || {} ) };
