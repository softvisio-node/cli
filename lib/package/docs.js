...
export default class Docs {
    ...
    async build () {
        ...
        try {
            ...
            warn = this.#checkMarkdown( options );
    ...
    async #initGitHubPages ( options ) {
        ...
        try {
            ...
            // pages created
            if ( res.ok ) {
                const path = options.location === "root" ? "/" : "/docs";

                if ( path === res.data.source.path ) {
                    throw result( [200, `Already initialized. Branch: ${res.data.source.branch}, path: ${res.data.source.path}.`] );
                }

                // update location
                else {
                    res = await github.updatePages( upstream.repoId, { "source": { "branch": "main", path } } );
                }
            }

            // create pages
            else {
                res = await github.createPages( upstream.repoId, "main", options.location === "root" ? "/" : "/docs" );
            }
        }
        catch ( e ) {
            res = result.catch( e );
