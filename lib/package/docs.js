...
export default class Docs {
    ...
    async #buildAppSchemas ( options ) {
        ...
        const appLocation = url.pathToFileURL( this.#rootPackage.root ),
            componentsUrl = resolve( "@softvisio/core/app/components", appLocation, { "url": true } ),
            Components = ( await import( componentsUrl ) ).default,
            components = new Components( appLocation );

        const schema1 = components.getApiSchema().data;
        console.log( schema1 );
        console.log( await schema1.generate( options.app.api ) );
