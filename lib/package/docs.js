...
export default class Docs {
    ...
    async #checkMarkdown ( options, { lint } = {} ) {
        ...
        for ( const file of files ) {
            ...
            const markdown = md.transform( source, {
                "handlers": {
                    code ( node, parent, state, info ) {
                        ...
                        return md.defaultHandlers.code( node, parent, state, info );
                    },
                    text ( node, parent, state, info ) {
                        const value = md.nodeToString( node );
                        ...
                        return md.defaultHandlers.text( node, parent, state, info );
                    },
                },
            } );

            if ( source !== markdown ) fs.writeFileSync( this.locationPath + "/" + file, markdown );
        }
