    ...
    async build () {
        ...
        if ( config.api ) {
            ...
            for ( const source in config.api ) {
                ...
                // update files
                const dir = this.#rootPackage.root + "/" + DOCS_LOCATION + "/" + config.api[source].target;
        ...
        const fileTree = new fs.FileTree(),
            gitUpstream = this.#rootPackage.git.upstream;
        ...
        if ( config.updateReadme && fs.existsSync( readmePath ) ) {
            ...
            // XXX replace relative links
            const README = fs.readFileSync( readmePath );

            const options = {
                "docsLink": gitUpstream.getPagesURL(), // XXX link to the documentation site, con be overriden
                README,
            };
        ...
        const options = {
            "docsLink": "", // link to the documentation site
            "README": "", // README.md content, XXX substiture relative links
        };
