...
export default class Docs {
    ...
    async #init ( options ) {
        ...
        if ( !fs.existsSync( options.locationPath + "/_sidebar.md" ) ) {
            fileTree.add( { "path": "_sidebar.md", "content": await ejs.renderFile( fs.resolve( "#resources/templates/docs/_sidebar.md", import.meta.url ), options ) } );
        }

        // generate .docs.config.yaml
        if ( !fs.existsSync( options.locationPath + "/_sidebar.md" ) ) {
            fileTree.add( { "path": ".docs.config.yaml", "content": await ejs.renderFile( fs.resolve( "#resources/templates/docs/.docs.config.yaml", import.meta.url ), options ) } );
        }
    ...
    async #buildDefault ( options ) {
        ...
        // lint index.html
        const res = await new LintFile( options.locationPath + "/" + "index.html", { "data": index } ).run( "lint" );
        if ( !res.ok ) throw res;
        index = res.data;
    ...
    async #buildAPISchemas ( options, type ) {
        ...
        for ( const filename in fileTree.files ) {
            const file = new LintFile( options.locationPath + "/" + filename );
