...
export default class Publish {
    ...
    async run () {
        ...
        // check, that new version or pre-release base isn't already released
        ...
        for ( const version of Object.values( id.versions ) ) {
            ...
            // prohibit to release v1.0.0 -> v2.0.0-alpha.1 if v2.0.0-beta.1 is already released on the other branch
            if ( this.#newVersion.isPreRelease && version.isPreRelease && this.#newVersion.base.toString() === version.base.toString() && version.gte( this.#newVersion ) ) {
                return result( [500, `version "${version}" is already released on the other branch, you need to merge first`] );
            }
