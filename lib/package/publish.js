...
export default class Publish {
    ...
    constructor ( pkg, preRelease, force = false ) {
        this.#pkg = pkg;
...
You need to merge with the "${latestPreRelease.toVersionString()}" first.`,
        ...
        // update linked packages versions
    ...
    async #updateChangelog ( changes ) {
        ...
        if ( !this.#force ) {
            ...
            var res = childProcess.spawnSync( process.env.EDITOR, [tmp.path], { "stdio": "inherit", "shell": true } );
        ...
        // write
        fs.writeFileSync( this.#pkg.root + "/CHANGELOG.md", this.#upstream ? this.#upstream.linkifyMessage( log ) : log );
