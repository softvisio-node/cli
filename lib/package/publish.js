...
export default class Publish {
    ...
    async run () {
        ...
        var state = await git.getState();
        if ( !state.ok ) return state;
        state = state.data;

        // check branch
        // if ( !state.branch ) return result( [500, `project is on tbe detached head`] );

        // check for uncommited changes
        if ( state.isDirty ) return result( [500, `working copy or sub-repositories has uncommited changes or untracked files`] );

        // check distance from the last release
        if ( state.currentVersion && !state.currentVersionDistance && ( await confirm( "No changes since the latest release. Continue?", ["n", "y"] ) ) === "n" ) return result( [500, "terminated"] );

        this.#version = state.currentVersion;
        ...
        if ( state.releases.has( this.#newVersion ) ) {
            return result( [
                500,
...
You need to merge with the "${this.#newVersion.toVersionString()}" first.`,
        ...
        if ( state.releases.has( this.#newVersion.base ) ) {
            return result( [
                500,
...
You need to merge first.`,
        ...
        this.#latestTag = !this.#newVersion.isPreRelease && this.#newVersion.gt( state.releases.latest ) ? "latest" : "";
        this.#nextTag = this.#newVersion.gt( state.releases.next ) ? "next" : "";

        const latestPreRelease = state.releases.getLatestPreRelease( this.#newVersion );
...
You need to merge with the "${latestPreRelease.toVersionString()}" first.`,
        ...
        if ( this.#upstream ) {
            while ( 1 ) {
                ...
                const params = ["push", "--atomic", "--force", "origin", state.branch, this.#newVersion.toVersionString(), this.#nextTag, this.#latestTag].filter( param => param != null && param !== "" );
