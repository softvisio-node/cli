    ...
    async run () {
        ...
        if ( latestPreRelease ) {
            ...
            if ( !this.#version.eq( latestPreRelease ) ) {
                return result( [500, `New release "${this.#newVersion}" must be the direct child of the release "${latestPreRelease}, which is already released on the other branch". You need to merge first.`] );
            }
        }
