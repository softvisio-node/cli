...
export default class Publish {
    ...
    async run () {
        ...
        // XXX
        // check, that version and it's base version are not already released on the other branch
        // if ( this.#version.base + "" !== this.#newVersion.base + "" ) {
        //     const newBase = this.#newVersion.base.toVersionString();

        //     for ( const version of Object.values( id.versions ) ) {
        //         if ( version.base.toVersionString() === newBase ) {
        //             return result( [500, `Version "${newBase}" is already released on other branch. You need to merge branches first`] );
        //         }
        //     }
        // }
        ...
        if ( !this.#newVersion.isPreRelease ) {
            for ( const pkg of [this.#pkg, ...packages] ) {
                ...
                if ( !res.ok ) {
                    console.log( ansi.warn( `One of releasing packages has pre-released dependencies. It is recommended to use pre-release tag.` ) );
