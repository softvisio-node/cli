...
export default class Publish {
    ...
    async run () {
        ...
        if ( !this.#releaseType ) {
            if ( changes.breaking ) this.#releaseType = "major";
            else if ( changes.feat ) this.#releaseType = "minor";
            else this.#releaseType = "patch";
        }
        ...
        if ( this.#version.isPreRelease ) {
            ...
            if ( newPrerelease ) {
                ...
                if ( this.#version.gte( this.#newVersion ) ) {
                    return result( [500, `Pre-release tag "${this.#preReleaseTag}" should be greater than current tag "${this.#version.prerelease[0]}".`] );
                }
            }
            else {
...
`;
    ...
    async #getChanges () {
        ...
        return result( 200, parseChanges( commits.data ) );
    }
}
