...
export default class Publish {
    ...
    async run () {
        ...
        // check branch
        // if ( !id.branch ) return result( [500, `project is on tbe detached head`] );
        ...
        if ( id.versions[this.#newVersion] ) return result( [500, `Version "${this.#newVersion}" is already released on the other branch. You need to merge first.`] );

        // check, that pre-release base isn't already released
        if ( id.versions[this.#newVersion.base] ) return result( [500, `Version "${this.#newVersion.base}" is already released on the other branch. You need to merge first.`] );

        // define tags, pre-release version can't have "latest" tag
        this.#latestTag = this.#newVersion.isPreRelease ? "" : "latest";
        this.#latestMajorTag = `latest.${this.#newVersion.major}`;
        ...
        for ( const version of Object.values( id.versions ) ) {
            ...
            if ( version.isPreRelease && version.base.eq( this.#newVersion.base ) ) {

                // version is the latest pre-release of the new version
                if ( !latestPreRelease || version.gt( latestPreRelease ) ) latestPreRelease = version;
            }

            // new version is not latest
            if ( version.gt( this.#newVersion ) ) this.#latestTag = "";

            // new version is not latest in the major branch
            if ( this.#newVersion.major === version.major && version.gt( this.#newVersion ) ) this.#latestMajorTag = "";
        }

        // new release has pre-release
        if ( latestPreRelease ) {

            // new pre-relesae must be greater than latest pre-release
            // if ( this.#newVersion.isPreRelease && latestPreRelease.gt( this.#newVersion ) ) {
            // return result( [500, `New release must be the direct child of the release "${latestPreRelease}, which is already released on the other branch". You need to merge first.`] );
            // }

            // new version must be the direct child of the latest pre-release of the new version
            if ( !this.#version.eq( latestPreRelease ) ) {
                return result( [500, `New release must be the direct child of the release "${latestPreRelease}", which is already released on the other branch. You need to merge with the "${latestPreRelease}" first.`] );
            }
        }
        ...
        if ( this.#latestMajorTag ) {
            res = await git.run( "tag", this.#latestMajorTag, "--force" );
            if ( !res.ok ) return res;
        }
        ...
        if ( this.#upstream ) {
            while ( 1 ) {
                ...
                res = await git.run( "push", "--atomic", "origin", id.branch, this.#newVersion.toVersionString(), this.#latestMajorTag, "latest", "--force" );
        ...
        await this.#pkg.publishNPM( this.#latestTag, this.#latestMajorTag );

        // publish linked workspaces
        for ( const pkg of packages ) {
            if ( !pkg.isPrivate ) await pkg.publishNPM( this.#latestTag, this.#latestMajorTag );
        }
    ...
    // private
    get #tags () {
        const tags = [this.#latestTag, this.#latestMajorTag].filter( tag => tag ).join( ", " );
