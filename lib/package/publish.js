...
export default class Publish {
    ...
    async run () {
        ...
        if ( id.releases.has( this.#newVersion ) ) return result( [500, `Version "${this.#newVersion}" is already released on the other branch.\nYou need to merge with the "${this.#newVersion.toVersionString()}" first.`] );

        // check, that pre-release base isn't already released
        if ( id.releases.has( this.#newVersion.base ) ) return result( [500, `Version "${this.#newVersion.base}" is already released on the other branch. You need to merge first.`] );
        ...
        for ( const version of id.releases.releases ) {
        ...
        if ( latestPreRelease ) {
            ...
            if ( !this.#version.eq( latestPreRelease ) ) {
                return result( [500, `New release must be the direct child of the release "${latestPreRelease}", which is already released on the other branch.\nYou need to merge with the "${latestPreRelease.toVersionString()}" first.`] );
            }
        }
