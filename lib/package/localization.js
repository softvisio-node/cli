    ...
    async update () {
        ...
        for ( const poFilePath of poFiles ) {
            ...
            for ( const source of sources ) {
                ...
                if ( extracted[poFilePackageRoot][source] == null ) {
                    const res = getText.extract( souceAbsPath, sourceRelPath );

                    if ( !res.ok ) return res;

                    extracted[poFilePackageRoot][source] = res.data || false;
                }

                if ( extracted[poFilePackageRoot][source] ) extractedMessages.addMessages( extracted[poFilePackageRoot][source] );
            }
