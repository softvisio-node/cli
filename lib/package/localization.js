...
export default class {
    ...
    async update () {
        ...
        for ( const file of pofiles ) {
            const absFile = path.join( this.#pkg.root, file );

            const poFile = PoFile.fromFile( absFile );

            if ( !poFile.searchPath ) continue;

            let poFileRoot = path.dirname( absFile );
            while ( true ) {
            ...
            const prefix = path.relative( poFileRoot, path.dirname( absFile ) ).replaceAll( "\\", "/" );
            ...
            fs.writeFileSync( absFile, poFile.toString() );
        }
