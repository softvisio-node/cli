...
export default class Publish {
    ...
    async run () {
        ...
        res = await this.#pkg.git.getChangelog( {
            "release": true,
            "stable": this.#stable,
            "commitTypes": this.#pkg.cliConfig?.commit.types,
        } );
        if ( !res.ok ) return res;
        ...
        // confirm release non-significant changes
        if ( !this.#changelog.hasSignificantChanges && ( await confirm( "No significant changes since the previous release. Continue?", [ "yes", "[no]" ] ) ) !== "yes" ) return result( [ 500, "Terminated" ] );
