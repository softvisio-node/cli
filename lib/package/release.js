...
export default class Publish {
    ...
    async run () {
        ...
        try {
            ...
            this.#currentRelease = ( this.#changelog.previousRelease || new SemanticVersion( "0" ) ).increment( this.#changelog.hasBreakingChanges
                ? "major"
...
You need to merge first.`,
        ...
        this.#latestTag = !this.#currentRelease.isPreRelease && this.#currentRelease.gt( status.releases.lastStableVersion )
            ? "latest"
            : "";
        this.#nextTag = this.#currentRelease.gt( status.releases.lastVersion )
            ? "next"
            : "";
