...
export default class Publish {
    ...
    async run () {
        ...
        // get changes
        var changes = await this.#pkg.git.getChanges( [ this.#baseVersion, "HEAD" ] );
        if ( !changes.ok ) return changes;
        changes = changes.data;

        // define new version
        try {
            this.#newVersion = ( this.#previousVersion || new SemanticVersion( "0" ) ).increment( changes.hasBreakingChanges
                ? "major"
                : changes.hasFeatureChanges
                    ? "minor"
        ...
        if ( this.#baseVersion.isPreRelease && !this.#newVersion.isPreRelease ) {
            res = await this.#pkg.git.getRelease( { "stable": true } );
            if ( !res.ok ) return res;

            this.#baseVersion = res.data.version;

            // update changes
            changes = await this.#pkg.git.getChanges( [ this.#baseVersion, "HEAD" ] );
            if ( !changes.ok ) return changes;
            changes = changes.data;
        }
...
You need to merge first.`,
        ...
        // XXX
        const latestPreRelease = status.releases.getLastPreReleaseVersion( this.#newVersion );

        // XXX
        // new version must be the direct child of the latest pre-release of the new version
        if ( latestPreRelease && this.#previousVersion.ne( latestPreRelease ) ) {
            return result( [
                500,
...
You need to merge with the "${ latestPreRelease.versionString }" first.`,
        ...
        console.log( `Previous version: ${ ansi.ok( ` ${ this.#baseVersion.versionString } ` ) }${ this.#baseVersion.ne( this.#previousVersion )
        ...
        if ( this.#upstream ) {
            res = await repeatAction( async () => {
                ...
                const params = [ "push", "--atomic", "--force", "origin", status.head.branch, this.#newVersion.versionString, this.#nextTag, this.#latestTag ].filter( param => param != null && param !== "" );
    ...
    async #updateChangelog ( changes ) {
        var res,
            log = await changes.createChangeLog( {
                ...
                "previousReleaseVersion": this.#baseVersion,
                "releaseVerion": this.#newVersion,
            } );
