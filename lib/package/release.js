...
export default class Publish {
    ...
    async run () {
        ...
        // check branch
        // if ( !status.head.branch ) return result( [500, `Package is on tbe detached head`] );
        ...
        try {
            ...
            // XXX create GitRelease
            this.#currentRelease = ( this.#changelog.previousRelease || SemanticVersion.initialVersion ).increment( this.#changelog.hasBreakingChanges
                ? "major"
        ...
        if ( status.releases.has( this.#currentRelease ) ) {
            return result( [
                500,
                `Version "${ this.#currentRelease }" is already released.
You need to merge with the "${ this.#currentRelease.versionString }" first.`,
            ] );
        }

        // check, that pre-release base isn't already released
        if ( status.releases.has( this.#currentRelease.release ) ) {
            return result( [
                500,
                `Version "${ this.#currentRelease.release }" is already released.
You need to merge first.`,
            ] );
        }

        // define tags, pre-release version can't have "latest" tag
        this.#latestTag = !this.#currentRelease.isPreRelease && this.#currentRelease.gt( status.releases.lastStableVersion || SemanticVersion.initialVersion )
            ? "latest"
            : "";

        this.#nextTag = this.#currentRelease.gt( status.releases.lastVersion || SemanticVersion.initialVersion )
            ? "next"
            : "";

        // XXX
        const latestPreRelease = status.releases.getLastPreReleaseVersion( this.#currentRelease );

        // XXX
        // new version must be the direct child of the latest pre-release of the new version
        if ( latestPreRelease && this.#changelog.previousRelease.ne( latestPreRelease ) ) {
            return result( [
                500,
...
You need to merge with the "${ latestPreRelease.versionString }" first.`,
        ...
        console.log( `Previous version: ${ ansi.ok( ` ${ this.#changelog.previousRelease?.versionString || "-" } ` ) }` );
        console.log( `New version:      ${ ansi.error( ` ${ this.#currentRelease.versionString } ` ) }` );
        ...
        res = await repeatAction( async () => {
            ...
            const res = await git.exec( [ "commit", "-m", `chore: release ${ this.#currentRelease.versionString }` ] );
        ...
        res = await repeatAction( async () => {
            ...
            const res = await git.exec( [ "tag", "-a", this.#currentRelease.versionString, "-m", `Release version: ${ this.#currentRelease.versionString }` ] );
    ...
    async #updateChangelog () {
        var res,
            changelogText = await this.#changelog.createChangelog( {
        ...
        while ( true ) {
            if ( !this.#yes ) {
                ...
                fs.writeFileSync( tmp.path, changelogText );
                ...
                changelogText = fs.readFileSync( tmp.path, "utf8" );
            ...
            res = await lintFile( new File( {
                ...
                "buffer": changelogText,
            } ) );
            if ( !res.ok ) return result( [ 500, `Error linting "CHANGELOG.md"` ] );

            changelogText = res.data;

            console.log( `\n${ ansi.hl( "# Changelog:" ) }\n\n${ new Markdown( changelogText ).toString( { "ansi": true } ) }` );
        ...
        var fullChangelogText = "# Changelog\n\n" + changelogText;
        ...
        fs.writeFileSync( this.#pkg.root + "/CHANGELOG.md", this.#upstream
            ? this.#upstream.linkifyMessage( changelogText )
            : fullChangelogText );
