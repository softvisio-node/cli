...
export default class Publish {
    ...
    async run () {
        ...
        if ( this.#version.isNull ) {
            this.#newVersion = this.#version.increment( "major", this.#preRelease );
        }

        // major
        else if ( changes.breakingChanges.length ) {
            if ( this.#version.isPreRelease && this.#version.isMajor ) {
                this.#newVersion = this.#version.increment( "prerelease", this.#preRelease );
            }
            else {
                this.#newVersion = this.#version.increment( "major", this.#preRelease );
            }
        ...
        // minor
        else if ( changes.featureChanges.length ) {
            if ( this.#version.isPreRelease && !this.#version.isPatch ) {
                this.#newVersion = this.#version.increment( "prerelease", this.#preRelease );
            }
            else {
                this.#newVersion = this.#version.increment( "minor", this.#preRelease );
            }
        }
        ...
        // patch
            ...
            if ( this.#version.isPreRelease ) {
                ...
                this.#newVersion = this.#version.increment( "patch", this.#preRelease );
            }
        }

        // drop pre-release
        if ( this.#dropPreRelease ) this.#newVersion = this.#newVersion.release;
...
You need to merge with the "${ this.#newVersion.toVersionString() }" first.`,
        ...
        if ( status.releases.has( this.#newVersion.release ) ) {
            return result( [
                500,
                `Version "${ this.#newVersion.release }" is already released on the other branch.
