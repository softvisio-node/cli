...
export default class Publish {
    ...
    constructor ( pkg, { preRelease, yes, batch } = {} ) {
        this.#pkg = pkg;
        this.#preRelease = preRelease;
        this.#yes = Boolean( yes );
        this.#batch = Boolean( batch );
    ...
    async run () {
        ...
        // in batch mode ignore if package has no changes
        if ( !this.#changelog.hasChanges && this.#batch ) return result( 200 );
        ...
        if ( !this.#changelog.hasChanges ) {
            const res = await confirm( "No changes since the previous release.\nContinue the release process?", [ "yes", "[no]" ] );
        ...
        // confirm no notable changes
            ...
            const res = await confirm( "No notable changes since the previous release.\nContinue the release process?", [ "yes", "[no]" ] );
        ...
        // confirm release
            ...
            const res = await confirm( "Continue the release process?", [ "[yes]", "no" ] );
        ...
        if ( docs.isEnabled ) {
            ...
            res = await git.getWorkingTreeStatus();
            if ( !res.ok ) return res;
            ...
            if ( res.data.isDirty ) {
                ...
                res = await repeatAction( async () => {
                    ...
                    const res = await git.exec( [ "commit", "-m", `chore(docs): update docs` ] );
        ...
        // update changelog
        res = await this.#createChangelog();
        if ( !res.ok ) return res;
        ...
        res = await repeatAction( async () => {
            ...
            const res = await git.exec( [ "commit", "--cleanup=verbatim", "-m", `chore(release): release ${ this.#currentRelease.versionString }\n\n${ this.#changelogText }\n` ] );
        ...
        if ( this.#latestTag ) {
            res = await repeatAction( async () => {
                ...
                const res = await git.exec( [ "tag", "--force", "-a", "-m", "Latest stable release", this.#latestTag ] );
        ...
        if ( this.#nextTag ) {
            res = await repeatAction( async () => {
                ...
                const res = await git.exec( [ "tag", "--force", "-a", "-m", "Next release", this.#nextTag ] );
    ...
    async #createChangelog () {
        ...
        while ( true ) {
            ...
            console.log( `\n${ ansi.hl( "# Changelog:" ) }\n\n${ this.#changelog.convertMarkdownToText( changelogMarkdown, {
                "ansi": true,
                "linkify": true,
            } ) }\n` );
...
`.trim();
    ...
    async #editChangelog ( changelogMarkdown ) {
        ...
        const res = childProcess.spawnSync( process.env.EDITOR, [ `"${ tmp.path }"` ], {
            "shell": true,
            "stdio": "inherit",
        } );
