...
You need to merge with the "${ latestPreRelease.toVersionString() }" first.`,
        ...
        while ( true ) {
            ...
            res = await git.exec( [ "commit", "-m", `chore: release ${ this.#newVersion.toVersionString() }` ] );
        ...
        while ( true ) {
            ...
            res = await git.exec( [ "tag", "-a", this.#newVersion.toVersionString(), "-m", `Release version: ${ this.#newVersion.toVersionString() }` ] );
        ...
        if ( this.#latestTag ) {
            while ( true ) {
                ...
                res = await git.exec( [ "tag", this.#latestTag, "--force", "-m", "Latest stable release" ] );
        ...
        if ( this.#nextTag ) {
            while ( true ) {
                ...
                res = await git.exec( [ "tag", this.#nextTag, "--force", "-m", "Next release" ] );
        ...
        if ( this.#upstream ) {
            ...
            if ( this.#upstream.hosting === "github" && process.env.GITHUB_TOKEN ) {
                ...
                while ( true ) {
                    ...
                    res = await github.createRelease( this.#upstream.repositorySlug, this.#newVersion.toVersionString(), {
    ...
    async #updateChangelog ( changes ) {
        var res,
            log = await ejs.renderFile( resolve( "#resources/templates/changelog.md.ejs", import.meta.url ), {
                changes,
        ...
        while ( true ) {
            ...
            // confirm release
            if ( !this.#yes ) {
                res = await confirm( "Continue release process?", [ "edit", "[yes]", "no" ] );
