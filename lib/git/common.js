...
export default class Priority {
    ...
    get links () {
        if ( this.#links === undefined ) {
            const links = [ ...new Set( [ ...this.message.matchAll( /\b((?:[\w.-]\/[\w.-])?#\d+)\b/g ) ] ) ].sort( ( a, b ) => {
    ...
    get fixes () {
        if ( this.#fixes === undefined ) {
            const links = [ ...new Set( [ ...this.message.matchAll( /\b(?:close[ds]?|fix(?:es|ed)?|resolve(?:s|ds)?) *:? +((?:[\w.-]\/[\w.-])?#\d+)\b/g ) ] ) ].sort( ( a, b ) => {
                const [ aRepo, aId ] = a.split( "#" ),
                    [ bRepo, bId ] = b.split( "#" );
