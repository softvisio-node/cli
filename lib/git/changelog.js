...
export default class GitChangelog {
    ...
    constructor ( changes, { upstream, issuesUpstream, previousRelease, currentRelease } ) {
        this.#changes = changes;
        this.#upstream = upstream;
        this.#issuesUpstream = issuesUpstream;
    ...
    async createChangelog ( { currentRelease, text, ansi = true } = {} ) {
        ...
        if ( text ) {
            return this.#createText( changelog, { ansi } );
        }
        else {
            return changelog;
        }
...
`.trim();
    ...
    #createText ( markdown, { ansi } = {} ) {
        if ( this.#issuesUpstream ) {
            markdown = this.#issuesUpstream.linkifyMessage( markdown );
        }
