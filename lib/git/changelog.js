...
export default class GitChangelog {
    ...
    async createChangeLog ( { markdown } = {} ) {
        const changelog = await ejs.renderFile( resolve( "#resources/templates/changelog.md.ejs", import.meta.url ), {
            ...
            "previousReleaseVersion": this.#previousRelease,
            "releaseVerion": this.#currentRelease,
            "blocks": {
    ...
    createReport () {
        const report = `
Total changes:    ${ this.size || "-" }
Breaking changes: ${ this.breakingChanges.length || "-" }
Features:         ${ this.featureChanges.length || "-" }
Fixes:            ${ this.fixChanges.length || "-" }
Other:            ${ this.otherChanges.length || "-" }
`.trim();
