...
export default class Commit extends Common {
    ...
    get links () {
        if ( this.#links === undefined ) {
            const links = [ ...new Set( [ ...this.message.matchAll( /(?:^|\s)((?:[\w.-]+\/[\w.-]+)?#\d+)(?:\s|$)/g ) ].map( match => match[ 1 ] ) ) ].sort( this.constructor.compareLinks );
    ...
    get fixes () {
        if ( this.#fixes === undefined ) {
            const links = [ ...new Set( [ ...this.message.matchAll( /(?:^|\s)(?:close[ds]?|fix(?:es|ed)?|resolve(?:s|ds)?) *:? +((?:[\w.-]+\/[\w.-]+)?#\d+)(?:\s|$)/gi ) ].map( match => match[ 1 ] ) ) ].sort( this.constructor.compareLinks );
