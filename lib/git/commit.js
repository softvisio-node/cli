...
const COMMIT_TITLE_RE = /^(?<type>[\da-z-]+)(?:\((?<scope>[\da-z-]+)\))?(?<breaking>!)?: (?<subjectText>.+)/;
...
export default class Commit extends Priority {
    ...
    compare ( commit ) {
        ...
        return this.priority - commit.priority || this.type.localeCompare( commit.type ) || this.scope.localeCompare( commit.scope ) || this.subjectText.localeCompare( commit.subjectText ) || this.hash?.localeCompare( commit.hash ) || 0;
    }
}
