...
export class LintFile {
    ...
    constructor ( file, { processUnsupported, write, useDefaults, cache } = {} ) {
        ...
        this.#write = !!write;
        this.#cache = cache || {};
        this.#useDefaults = useDefaults ?? this.#hasCliConfig( this.#getAbsPath() );
    }
    ...
    async run ( action ) {
        ...
        if ( md5 !== this.#md5 ) {
            ...
            if ( this.#write && this.#getAbsPath() ) {
                await fs.promises.writeFile( this.#getAbsPath(), this.#data );
            }
        }
        else {
    ...
    async #lint () {
        ...
        prettierConfig = await prettier.resolveConfig( this.#getAbsPath(), {
            "editorconfig": false,
        } );

        // use default prettier config
        if ( !prettierConfig && this.#useDefaults ) {
            prettierConfig = {
                ...DEFAULT_PRETTIER_CONFIG,
            };
        }

        if ( prettierConfig && editorConfig ) {
        ...
        if ( this.#type ) {
            ...
            if ( prettierConfig && this.#type.prettier?.parser ) {
                ...
                prettierConfig.filepath = this.#getAbsPath();
    ...
    async #getEditorConfig () {
        ...
        const config = await editorconfig.parse( fullPath, {
        ...
        if ( !Object.keys( config ).length ) {
            ...
            // use default config
            return editorconfig.parse( path.join( RESOURCES_ROOT, path.basename( fullPath ) ), {
                "cache": this.#cache.editorConfig,
                "unset": true,
            } );
        }
    ...
    #getAbsPath () {
        if ( !this.#absPath ) {
            this.#absPath = path.resolve( this.#file.path );
        }
    ...
    #getFullPath () {
        if ( !this.#fullPath ) {
            if ( this.#type.extname && path.extname( this.#getAbsPath() ) !== this.#type.extname ) {
                this.#fullPath = this.#getAbsPath() + this.#type.extname;
            }
            else {
                this.#fullPath = this.#getAbsPath();
            }
        }
