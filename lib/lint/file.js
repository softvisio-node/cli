...
export class LintFile {
    ...
    constructor ( file, { processUnsupported, write, useDefaults, cache } = {} ) {
        ...
        this.#write = !!write;
        this.#cache = cache || {};
        this.#useDefaults = useDefaults ?? this.#hasCliConfig();
    }

    // properties
    get absPath () {
        if ( !this.#absPath ) {
            this.#absPath = path.normalize( path.resolve( this.#file.path ) );
        }
    ...
    async #lint () {
        ...
        prettierConfig = await prettier.resolveConfig( this.absPath, {
            "editorconfig": false,
        } );
    ...
    #hasCliConfig () {
        ...
        var dir = path.dirname( this.absPath );
