...
export class LintFile {
    ...
    constructor ( file, { processUnsupported, write, useDefaults, cache } = {} ) {
        this.#file = file;
        this.#processUnsupported = !!processUnsupported;
        this.#write = !!write;
        this.#cache = cache || {};
        this.#useDefaults = useDefaults ?? this.#hasCliConfig( this.#file.path );
    }
    ...
    // private
    async #lint () {
        const editorConfig = await this.#getEditorConfig();

        const hasPreittierConfig = await this.#hasPrettierConfig();

        var prettierConfig;

        // load prettier config
        if ( hasPreittierConfig ) {
            prettierConfig = await prettier.resolveConfig( this.#file.path, {
                "editorconfig": true,
            } );
        }
        ...
        if ( this.#type ) {
            ...
            // run prettier
            if ( prettierConfig && this.#type.prettier?.parser ) {
                ...
                prettierConfig.filepath = this.#file.path;
            ...
            // eslint
            if ( this.#type.eslint ) {
                res = await this.#runEslint( editorConfig?.indent_style === "tab" ? "tab" : editorConfig?.indent_size );

                if ( res.status >= 500 ) return res;
            }
        }

        // process editor config
        if ( editorConfig ) {
    ...
    async #runEslint ( indent ) {
        const fullPath = this.#file.path + ( this.#type.eslint.extname || "" ),
        ...
        try {
            ...
            if ( isIgnored ) return result( 200 );
        }
        catch ( e ) {
            if ( !this.#useDefaults ) {
                return result( 200 );
        ...
        // javascript
        if ( this.#type.prettier?.parser === "babel" || this.#type.prettier?.parser === "typescript" ) {
        ...
        // vue
        else if ( this.#type.prettier?.parser === "vue" ) {
            this.#data = this.#data.replace( /^\s*<!-- ----- SOURCE FILTER LOG BEGIN -----[\s\S]+?----- SOURCE FILTER LOG END ----- -->/m, "" );
    ...
    #hasCliConfig ( dir ) {
        ...
        this.#cache.cliConfig ||= {};

        while ( true ) {
            if ( dir in this.#cache.cliConfig ) return this.#cache.cliConfig[ dir ];

            if ( fs.existsSync( path.join( dir, "cli.config.yaml" ) ) ) {
                this.#cache.cliConfig[ dir ] = true;
            ...
            if ( dirname === dir ) {
                this.#cache.cliConfig[ dir ] = false;
    ...
    async #hasPrettierConfig () {
        this.#cache.prettierConfig ||= {};

        const dirname = path.dirname( this.#file.path );

        if ( dirname in this.#cache.prettierConfig ) {
            return this.#cache.prettierConfig[ dirname ];
        }

        const hasPrettierConfig = await prettier.resolveConfigFile( dirname );

        this.#cache.prettierConfig[ dirname ] = hasPrettierConfig;

        return hasPrettierConfig;
    }

    async #getEditorConfig () {
        this.#cache.editorConfig ||= new Map();

        const config = await editorconfig.parse( this.#file.path, {
            "cache": this.#cache.editorConfig,
            "unset": true,
        } );
        ...
        if ( !Object.keys( config ).length ) {
            ...
            // use default config
            return editorconfig.parse( path.join( RESOURCES_ROOT, path.basename( this.#file.path ) ), {
                "cache": this.#cache.editorConfig,
                "unset": true,
            } );
        }
