...
export class LintFile {
    ...
    constructor ( file, { cwd, processUnsupportedTypes, write, useDefaults, fix = true, log = true, cache } = {} ) {
        this.#file = file;
    ...
    async #runEslint ( { configs, editorConfig } ) {
        ...
        if ( eslintConfig === true ) {
            ...
            if ( editorConfig ) {
                ...
                options.overrideConfig.rules[ plugin + "eol-last" ] = [ "error", editorConfig.insert_final_newline ? "always" : "never" ];

                // override @stylistic/no-trailing-spaces
                options.overrideConfig.rules[ plugin + "no-trailing-spaces" ] = [
                    "error",
                    {
