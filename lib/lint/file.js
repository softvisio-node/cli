...
export class LintFile {
    ...
    constructor ( file, { type, cwd, processUnsupportedTypes, write, useEslint = true, fix = true, log = true, cache } = {} ) {
    ...
    async #lint () {
        ...
        // eslint
        if ( this.#useEslint && this.#type?.eslint ) {
    ...
    async #runMarkdown ( { configs, editorConfig } = {} ) {
        ...
        // use prettier to format unsupported code blocks
        res = await this.#runPrettier( {
            configs,
            editorConfig,
        } );
        ...
        for ( const node of nodes ) {
            const originalLanguage = node.type === "yaml"
                    ? "yaml"
                    : node.type === "toml"
                        ? "toml"
                        : node.lang,
                language = markdown.getCodeLanguage( originalLanguage );
            ...
            if ( language.type ) {
                ...
                res = await lintFile(
                    {
                        ...
                        "type": language.type,
                        "sources": markdown.nodeToString( node ),
                    },
                    {
                        "cwd": this.#cwd,
                        "log": false,
                ...
                if ( res.data ) {
                    node.value = res.data.trim();
                }
            }
        }

        this.#data = markdown.toMarkdown( {
            "indent": editorConfig.indent_style === "tab"
