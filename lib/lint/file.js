...
export class LintFile {
    ...
    async #lintMarkdown ( { configs, editorConfig } = {} ) {
        ...
        markdown.visit( "code", ( node, index, parent ) => {
            nodes.push( {
                "id": null,
                "language": node.lang,
                node,
        ...
        for ( const node of nodes ) {
            ...
            node.node.lang = node.id;

            let language = MARKDOWN_CODE_LANGUAGES[ node.language ];

            if ( !language ) {
                LNG: for ( const lng in MARKDOWN_CODE_LANGUAGES ) {
                    for ( const alias in MARKDOWN_CODE_LANGUAGES[ lng ].aliases || [] ) {
                        if ( node.language === alias ) {
                            language = MARKDOWN_CODE_LANGUAGES[ lng ];
            ...
            if ( language?.type ) {
                res = await new this.constructor(
                    new File( {
        ...
        for ( const node of nodes ) {
            ...
            this.#data.replace( node.id, node.language );
        }
    }
}
