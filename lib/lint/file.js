...
export class LintFile {
    ...
    async run ( action ) {
        ...
        res.meta.diagnostic = this.#diagnostic?.length
            ? this.#diagnostic
            : null;

        res.meta.parsingError ??= false;
    ...
    async #runPrettier ( { configs, editorConfig } ) {
        ...
        try {
            ...
            // sh parser
            if ( e.Pos ) {
                location.lnum = e.Pos.Line - 1;
                location.col = e.Pos.Col - 1;
            }

            // other parser
            else {
                location.lnum = e.loc.start.line - 1;
                location.col = e.loc.start.column - 1;
                location.source = e.cause.code;
            }

            this.#diagnostic = [
                {
                    "severity": "ERROR",
                    "message": ansi.remove( e.message ),
                    ...location,

                    // "message": ansi.remove( e.message.split( "\n" )[ 0 ] ),
                    // "code": "\n" + ansi.remove( e.codeFrame ),
                },
            ];

            const res = result( [ 500, "Parsing error" ] );

            res.meta.parsingError = true;
    ...
    async #runEslint ( { configs, editorConfig, fatalError } ) {
        ...
        if ( diagnostic.length ) this.#diagnostic = diagnostic;
