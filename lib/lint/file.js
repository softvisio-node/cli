...
const TYPES = {
    ...
    "application/x-sh": { "parser": "sh", "prettierPlugin": PrettierPluginSh },
    "text/xml": { "parser": "xml" },
...
export default class LintFile {
    ...
    async #lint () {
        ...
        if ( this.#type ) {
            ...
            // run prettier
            if ( this.#type.parser ) {
                if ( this.#type.prettierPlugin ) {
                    config.plugins ??= [];
                    config.plugins.push( this.#type.prettierPlugin );
                }
...
// |==========+==========+==========================================================================|
// | ERROR    |     10:8 | @softvisio/camelcase, Identifier 'PrettierSHPlugin' is not in camel case.|
// +----------+----------+--------------------------------------------------------------------------+
//
