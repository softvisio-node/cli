...
const validateCliConfig = new Ajv().compileFile( import.meta.resolve( "#resources/schemas/.softvisio-cli.schema.yaml" ) ),
    defaultCliConfig = readConfig( "#resources/.softvisio-cli.yaml", { "resolve": import.meta.url } );
    ...
    get cliConfig () {
        if ( !this.#cliConfig ) {
            ...
            if ( fs.existsSync( this.root + "/.softvisio-cli.yaml" ) ) {
                cliConfig = mergeObjects( {}, defaultCliConfig, readConfig( this.root + "/.softvisio-cli.yaml" ) );
            }
            else {
    ...
    hasPreReleaseDependencies () {
        ...
        for ( const name of [ "dependencies", "devDependencies", "peerDependencies", "optionalDependencies" ] ) {
            ...
            for ( const dependencyName in config[ name ] ) {
                ...
                if ( dependencyRange === "latest" ) {
                    return result( 200 );
                }
                else if ( dependencyRange === "next" ) {
                    return result( [ 500, `Package "${ this.name }" has pre-release dependency "${ dependencyName }@${ config[ name ][ dependencyName ] }"` ] );
                }
