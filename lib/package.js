...
export default class Package {
    ...
    async testPlan ( options = {} ) {
        if ( !fs.existsSync( this.root + "/__tests__" ) ) {
            console.log( `Tests plan for package "${this.name}": ${ansi.warn( " NOT FOUND " )}` );
        }
        ...
        console.log( `Tests plan for package "${this.name}"` );
    ...
    async jest ( options = {} ) {
        ...
        const args = [env.root + "/node_modules/jest/bin/jest.js", "--passWithNoTests", "--rootDir", "__tests__", "--testPathIgnorePatterns", "bench"];

        if ( options.bail ) args.push( "--bail" );
        if ( options.silent ) args.push( "--silent" );
        if ( !options.stackTrace ) args.push( "--noStackTrace" );
        ...
        if ( options.jestArgv ) args.push( ...options.jestArgv );
