...
export default class Package {
    ...
    async release ( preRelease, force ) {
        const { "default": Release } = await import( "./package/publish.js" );

        const release = new Release( this, preRelease, force );
    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot } = {} ) {
        ...
        // get git status
        var res = await this.git.getStatus( {
            "pushStatus": false,
            "releases": false,
        } );
        ...
        for ( const pkg of packages ) {
            ...
            if ( pkg.hasCliConfig ) {
                ...
                // "release" script
                if ( pkg.isGitPackage && !pkg.cliConfig.private ) {
                    config.scripts ??= {};
                    config.scripts.release = "softvisio-cli package release";
                }
                else {
                    delete config.scripts?.release;
                }
            ...
            fileTree.add( {
                "path": ( pkg.monoRepositoryDirectory || "" ) + "/package.json",
    ...
    async checkOutdatedDependencies ( { update, updateMissing, commit, force } = {} ) {
        var res = childProcess.spawnSync( "npm", [ "outdated", "--json" ], {
            "cwd": this.root,
            "stdio": "pipe",
            "shell": true,
        } );
        ...
        try {
            var outdatedDependencies = JSON.parse( res.stdout );
        }
        catch {
            return result( 500 );
        }

        var hasMissingDependencies, updatable;

        const updateDependencies = [];

        // current, wanted, latest
        for ( const [ name, spec ] of Object.entries( outdatedDependencies ) ) {
            ...
            if ( spec.wanted === spec.latest ) {
                ...
                // skip missing updatable dependencies
                if ( !updateMissing && !spec.current ) continue;
            }
            else {
        ...
        if ( hasMissingDependencies && !updateMissing ) updatable = false;
        ...
        table.add( ...updateDependencies );

        table.end();

        // nothing to update
        if ( !update || !updatable ) return result( 200 );
    ...
    // XXX enable tests via npm, node >= 22
    test ( { log } = {} ) {
        var res;
        ...
        if ( !this.config.scripts?.test ) {
            ...
            res = childProcess.spawnSync( "node", [ "--test" ], {
        ...
        NPM: {
            ...
            for ( const pkg of [ this, ...this.subPackages ] ) {
                if ( pkg.config.dependencies || pkg.config.devDependencies || pkg.config.peerDependencies ) {
                    directories.push( "/" + ( pkg.monoRepositoryDirectory || "" ) );
                }
            }
