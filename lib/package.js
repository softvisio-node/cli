...
export default class Package {
    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot, github } = {} ) {
        ...
        if ( github && upstream.isGitHub && process.env.GITHUB_TOKEN ) {
            ...
            let res = await gitHubApi.getRepository( upstream.repoId );
            if ( !res.ok ) return res;

            let data;

            // description
            if ( this.config.description !== res.data.description ) {
                data ??= {};
            ...
            if ( res.data.security_and_analysis ) {
                ...
                if ( repository.secretScanningNonProviderPatterns !== null && repository.secretScanningNonProviderPatterns !== res.data.security_and_analysis.secret_scanning_non_provider_patterns.status ) {
                    data ??= {};
                    data.security_and_analysis ??= {};
                    data.security_and_analysis.secret_scanning_non_provider_patterns = {
                        "status": repository.secretScanningNonProviderPatterns,
                    };
                }
            }
