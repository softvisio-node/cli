    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot, github } = {} ) {
        ...
        if ( github && upstream.isGitHub && process.env.GITHUB_TOKEN ) {
            ...
            if ( repository.dependabotsecurityupdates != null ) {
                res = await gitHubApi.getDependabotsecurityupdatesStatus( upstream.repoId );
                if ( !res.ok ) return res;

                if ( repository.dependabotsecurityupdates !== res.data.enabled ) {
                    updated = true;

                    if ( repository.dependabotsecurityupdates ) {
                        res = await gitHubApi.enableDependabotsecurityupdate( upstream.repoId );
                    }
                    else {
                        res = await gitHubApi.disableDependabotsecurityupdate( upstream.repoId );
                    }
            ...
            // res = await gitHubApi.getPrivateVulnerabilityReportingStatus( upstream.repoId );
            // console.log( res );

            // res = await gitHubApi.getVulnerabilityAlertsStatus( upstream.repoId );
            // console.log( res );
