...
export default class Package {
    ...
    get rootSlug () {
        if ( this.#rootSlug === undefined ) {
            if ( this.rootPackage ) {
                this.#rootSlug = path.posix.relative( this.rootPackage.root, this.root ).replaceAll( "\\", "/" );
            }
            else {
                this.#rootSlug = null;
            }
    ...
    get parentSlug () {
        if ( this.#parentSlug === undefined ) {
            if ( this.parentPackage ) {
                this.#parentSlug = path.posix.relative( this.parentPackage.root, this.root ).replaceAll( "\\", "/" );
            }
            else {
    ...
    get config () {
        if ( !this.#config ) this.#config = readConfigSync( this.root + "/package.json" );
    ...
    get cliConfig () {
        if ( !this.#cliConfig ) {
            ...
            if ( this.hasCliConfig ) {
                cliConfig = mergeObjects( {}, defaultCliConfig, readConfigSync( this.root + "/cli.config.yaml" ) );
            }
            else {
    ...
    get workspaceSlug () {
        ...
        return path.posix.relative( workspace, this.root );
    }
    ...
    hasPreReleaseDependencies () {
        ...
        for ( const name of [ "dependencies", "devDependencies", "peerDependencies", "optionalDependencies" ] ) {
            ...
            for ( const dependencyName in config[ name ] ) {
                ...
                dependencyRange = new SemverRange( dependencyRange );
    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot, github } = {} ) {
        env.loadUserEnv();
        ...
        // github
        if ( github && upstream.isGitHub && process.env.GITHUB_TOKEN ) {
            const gitHubApi = new GitHubApi( process.env.GITHUB_TOKEN ),
                repository = this.cliConfig.meta.repository;

            let res, data;

            // repository settings
            res = await gitHubApi.getRepository( upstream.repoId );
            if ( !res.ok ) return res;

            const currentData = res.data;

            // description
            if ( this.config.description && this.config.description !== res.data.description ) {
                data ??= {};
                data.description = this.config.description;
            }

            // homepage
            if ( this.config.homepage !== res.data.homepage ) {
                data ??= {};
                data.homepage = this.config.homepage;
            }

            // archived
            if ( repository.archived !== null && repository.archived !== res.data.archived ) {
                data ??= {};
                data.archived = repository.archived;
            }

            // private
            if ( repository.private !== null && repository.private !== res.data.private ) {
                data ??= {};
                data.private = repository.private;
            }

            // visibility
            if ( repository.visibility !== null && repository.visibility !== res.data.visibility ) {
                data ??= {};
                data.visibility = repository.visibility;
            }

            // issues
            if ( repository.hasIssues !== null && repository.hasIssues !== res.data.has_issues ) {
                data ??= {};
                data.has_issues = repository.hasIssues;
            }

            // projects
            if ( repository.hasProjects !== null && repository.hasProjects !== res.data.has_projects ) {
                data ??= {};
                data.has_projects = repository.hasProjects;
            }

            // wiki
            if ( repository.hasWiki !== null && repository.hasWiki !== res.data.has_wiki ) {
                data ??= {};
                data.has_wiki = repository.hasWiki;
            }

            // discussions
            if ( repository.hasDiscussions !== null && repository.hasDiscussions !== res.data.has_discussions ) {
                data ??= {};
                data.has_discussions = repository.hasDiscussions;
            }

            // default branch
            if ( repository.defaultBranch !== null && repository.defaultBranch !== res.data.default_branch ) {
                data ??= {};
                data.default_branch = repository.defaultBranch;
            }

            // forking
            if ( repository.allowForking !== null && repository.allowForking !== res.data.allow_forking ) {
                data ??= {};
                data.allow_forking = repository.allowForking;
            }

            // security and analysis
            if ( res.data.security_and_analysis ) {

                // secret scanning
                if ( repository.secretScanning !== null && repository.secretScanning !== res.data.security_and_analysis.secret_scanning.status ) {
                    data ??= {};
                    data.security_and_analysis ??= {};
                    data.security_and_analysis.secret_scanning = {
                        "status": repository.secretScanning,
                    };
                }

                // secret scanning push protection
                if ( repository.secretScanningPushProtection !== null && repository.secretScanningPushProtection !== res.data.security_and_analysis.secret_scanning_push_protection.status ) {
                    data ??= {};
                    data.security_and_analysis ??= {};
                    data.security_and_analysis.secret_scanning_push_protection = {
                        "status": repository.secretScanningPushProtection,
                    };
                }
            }
            ...
            if ( data ) {
                ...
                res = await gitHubApi.updateRepository( upstream.repoId, data );
                if ( !res.ok ) return res;
            }

            // vulnerability alerts
            if ( repository.vulnerabilityAlerts != null ) {
                res = await gitHubApi.getVulnerabilityAlertsEnabled( upstream.repoId );
                if ( !res.ok ) return res;

                if ( repository.vulnerabilityAlerts !== res.data.enabled ) {
                    res = await gitHubApi.setVulnerabilityAlertsEnabled( upstream.repoId, repository.vulnerabilityAlerts );
                    if ( !res.ok ) return res;
            ...
            if ( repository.dependabotsecurityupdates != null ) {
                res = await gitHubApi.getDependabotsecurityupdatesEnabled( upstream.repoId );
                if ( !res.ok ) return res;

                if ( repository.dependabotsecurityupdates !== res.data.enabled ) {
                    res = await gitHubApi.setDependabotsecurityupdateEnabled( upstream.repoId, repository.dependabotsecurityupdates );
                    if ( !res.ok ) return res;
            ...
            if ( repository.privateVulnerabilityReporting != null && !currentData.private ) {
                res = await gitHubApi.getPrivateVulnerabilityReportingEnabled( upstream.repoId );
                if ( !res.ok ) return res;

                if ( repository.privateVulnerabilityReporting !== res.data.enabled ) {
                    res = await gitHubApi.setPrivateVulnerabilityReportingEnabled( upstream.repoId, repository.privateVulnerabilityReporting );
                    if ( !res.ok ) return res;
