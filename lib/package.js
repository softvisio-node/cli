...
export default class Package {
    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot, github } = {} ) {
        ...
        if ( github && upstream.isGitHub && process.env.GITHUB_TOKEN ) {
            ...
            // archived
            if ( repositorySettings.archived !== null && repositorySettings.archived !== res.data.archived ) {
                data ??= {};
                data.archived = repositorySettings.archived;
            }

            // private
            if ( repositorySettings.private !== null && repositorySettings.private !== res.data.private ) {
                data ??= {};
                data.private = repositorySettings.private;
            }

            // visibility
            if ( repositorySettings.visibility !== null && repositorySettings.visibility !== res.data.visibility ) {
                data ??= {};
                data.visibility = repositorySettings.visibility;
            }

            // issues
            if ( repositorySettings.hasIssues !== null && repositorySettings.hasIssues !== res.data.has_issues ) {
                data ??= {};
                data.has_issues = repositorySettings.hasIssues;
            }

            // projects
            if ( repositorySettings.hasProjects !== null && repositorySettings.hasProjects !== res.data.has_projects ) {
                data ??= {};
                data.has_projects = repositorySettings.hasProjects;
            }

            // wiki
            if ( repositorySettings.hasWiki !== null && repositorySettings.hasWiki !== res.data.has_wiki ) {
                data ??= {};
                data.has_wiki = repositorySettings.hasWiki;
            }

            // discussions
            if ( repositorySettings.hasDiscussions !== null && repositorySettings.hasDiscussions !== res.data.has_discussions ) {
                data ??= {};
                data.has_discussions = repositorySettings.hasDiscussions;
            }

            // default branch
            if ( repositorySettings.defaultBranch !== null && repositorySettings.defaultBranch !== res.data.default_branch ) {
                data ??= {};
                data.default_branch = repositorySettings.defaultBranch;
            }

            // forking
            if ( repositorySettings.allowForking !== null && repositorySettings.allowForking !== res.data.allow_forking ) {
                data ??= {};
            ...
            if ( res.data.security_and_analysis ) {
                ...
                // secret scanning
                if ( repositorySettings.secretScanning !== null && repositorySettings.secretScanning !== res.data.security_and_analysis.secret_scanning.status ) {
                    data ??= {};
                ...
                if ( repositorySettings.secretScanningPushProtection !== null && repositorySettings.secretScanningPushProtection !== res.data.security_and_analysis.secret_scanning_push_protection.status ) {
                    data ??= {};
                    data.security_and_analysis ??= {};
            ...
            if ( repositorySettings.vulnerabilityAlerts != null ) {
                ...
                if ( repositorySettings.vulnerabilityAlerts !== res.data.enabled ) {
                    res = await gitHubApi.setVulnerabilityAlertsEnabled( upstream.repoId, repositorySettings.vulnerabilityAlerts );
                    if ( !res.ok ) return res;
            ...
            if ( repositorySettings.dependabotsecurityupdates != null ) {
                ...
                if ( repositorySettings.dependabotsecurityupdates !== res.data.enabled ) {
                    res = await gitHubApi.setDependabotsecurityupdateEnabled( upstream.repoId, repositorySettings.dependabotsecurityupdates );
                    if ( !res.ok ) return res;
            ...
            if ( repositorySettings.privateVulnerabilityReporting != null && !currentData.private ) {
                ...
                if ( repositorySettings.privateVulnerabilityReporting !== res.data.enabled ) {
                    res = await gitHubApi.setPrivateVulnerabilityReportingEnabled( upstream.repoId, repositorySettings.privateVulnerabilityReporting );
                    if ( !res.ok ) return res;
