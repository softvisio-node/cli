...
export default class Package {
    ...
    // static
    static isPackageRoot ( dir ) {
        return env.isPackageRoot( dir );
    }

    static isGitPackageRoot ( dir ) {
        return env.isGitPackageRoot( dir );
    }

    static findGitPackage ( root ) {
        root = env.findGitPackageRoot( root );

        if ( root ) return new this( root.replaceAll( "\\", "/" ) );
    }

    static findPackage ( root ) {
    ...
    get isPackage () {
        if ( this.#isPackage == null ) this.#isPackage = this.constructor.isPackageRoot( this.root );

        return this.#isPackage;
    }

    get isGitPackageRoot () {
        this.#isGitPackageRoot ??= env.isGitPackageRoot( this.root );

        return this.#isGitPackageRoot;
    }
    ...
    get cliConfig () {
        if ( !this.#cliConfig ) {
            if ( fs.existsSync( this.root + "/.softvisio-cli.yaml" ) ) {
                this.#cliConfig = readConfig( this.root + "/.softvisio-cli.yaml" );
            }
            else {
                this.#cliConfig = {};
            }
    ...
    get workspaces () {
        BREAK: if ( this.#workspaces == null ) {
            ...
            for ( const pattern of workspaces ) {
                ...
                if ( this.constructor.isPackageRoot( root ) ) {
                    this.#workspaces.push( new this.constructor( root, {
                        "rootPackage": this.isGitPackageRoot ? this : this.rootPackage,
                        "parentPackage": this,
                    } ) );
                }
            }
    ...
    get commitsConfig () {
        if ( !this.#commitsConfig ) {
            ...
            this.#commitsConfig = {
                "strict": this.config.cliConfig?.strict ?? defaultConfig.strict,
                "types": new Set( [ ...defaultConfig.types, ...( this.config.cliConfig?.types || [] ) ] ),
                "requireScope": this.config.cliConfig?.requireScope ?? defaultConfig.requireScope,
                "scopes": new Set( [ ...defaultConfig.scopes, ...( this.config.cliConfig?.scopes || [] ) ] ),
            };
        }
