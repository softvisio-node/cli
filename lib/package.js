...
export default class Package {
    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot, github } = {} ) {
        ...
        if ( github && this.git.upstream.isGitHub && process.env.GITHUB_TOKEN ) {

            // XXX
            const gitHubApi = new GitHubApi( process.env.GITHUB_TOKEN );

            let res = await gitHubApi.getRepository( this.git.upstream.repoId );
            if ( !res.ok ) return res;
            ...
            res = await gitHubApi.updateRepository( this.git.upstream.repoId, data );
            if ( !res.ok ) return res;
        }
