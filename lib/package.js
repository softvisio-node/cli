...
export default class Package {
    ...
    get hasDependencies () {
        return Object.keys( this.config.dependencies || {} ).length || Object.keys( this.config.devDependencies || {} ).length || Object.keys( this.config.peerDependencies || {} ).length || Object.keys( this.config.optionalDependencies || {} ).length;
    }
    ...
    async updateDependencies ( { all, updatable, outdated, workspace, missing, install, reinstall, reinstallLinked, force, commit, quiet, yes, cache, outdatedDependencies, logHeader } = {} ) {
        if ( !this.hasDependencies ) return result( 200 );
        ...
        if ( install && reinstallLinked && !reinstall ) {
            reinstall = await this.#hasLinkedPackages();
        }
        ...
        if ( !force && this.isDependenciesLocked ) {
            ...
            try {
                ...
                if ( res.data.isDirty ) {
                    ...
                    if ( add.length ) {
                        ...
                        // add changes
                        res = await this.git.exec( [ "add", ...add ] );
                        if ( !res.ok ) throw res;

                        // commit changes
                        res = await this.git.exec( [ "commit", "-m", "chore: update package dependencies" ] );
                        if ( !res.ok ) throw res;
    ...
    async #updateMetadata ( { dependabot, commit } = {} ) {
        ...
        for ( const pkg of packages ) {
            ...
            // homepage
            if ( !config.homepage ) {
                const homepage = ( this.docs.isEnabled && upstream.docsUrl ) || upstream.homeUrl;
        ...
        if ( updated ) {
            ...
            if ( commit ) {
                ...
                // commit changes
                res = await this.git.exec( [ "commit", "-m", "chore: update package metadata" ] );
                if ( !res.ok ) return res;
