    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot } = {} ) {
        ...
        for ( const pkg of packages ) {
            ...
            if ( this.hascliconfig ) {
                ...
                // publish script
                if ( this.isGitPackage && !this.cliConfig.private ) {
                    config.scripts ??= {};
                    config.scripts.publish = "softvisio-cli package publish";
                }
                else {
                    delete config.scripts?.publish;
                }

                // test script
                if ( fs.existsSync( this.root + "/_tests" ) ) {
                    config.scripts ??= {};
