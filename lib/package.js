...
export default class Package {
    ...
    async updateMetadata ( { commit, push, homepage, author, license, dependabot } = {} ) {
        ...
        for ( const pkg of packages ) {
            ...
            // homepage
            if ( !config.homepage || homepage ) config.homepage = ( pkg.docs.isExists && upstream.docsUrl ) || upstream.homeUrl;
            ...
            // author
            if ( !config.author || author ) config.author = process.env.META_AUTHOR;

            writeConfig( pkg.root + "/package.json", config, {
                "readable": true,
            } );
        }
        ...
        if ( dependabot ) {
            ...
            if ( upstream.isGithub ) {
                ...
                for ( const subpackage of this.subPackages ) {
                    directories.push( "/" + subpackage.monoRepositoryDirectory );
                }
                ...
                console.log( yaml.stringify( dependabot ) );
