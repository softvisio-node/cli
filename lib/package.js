...
export default class Package {
    ...
    async updateMetadata ( { archived, repository, homepage, author, license, dependabot, commit, log } = {} ) {
        var res,
            report = "";

        // set archived status
        if ( archived != null ) {
            res = await this.setRepositoryArchived( archived );

            ( report += "Set archived status: " ) + res.ok
                ? ( res.data.updated
                    ? ansi.ok( " Updated " )
                    : "Not modified" )
        ...
        if ( repository ) {
            ...
            ( report += "\nConfigure upstream repository:" ), res.ok
                ? ( res.data.updated
                    ? ansi.ok( " Updated " )
                    : "Not modified" )
                : ansi.error( " " + res.statusText + " " );
        ...
        // update metadata
        {
            res = await this.#updateMetadata( { homepage, author, license, dependabot, commit } );

            ( report += "\nUpdate metadata:" ), res.ok
                ? ( res.data.updated
                    ? ansi.ok( " Updated " )
                    : "Not modified" )
                : ansi.error( " " + res.statusText + " " );
    ...
    async updateDependencies ( { all, updatable, outdated, workspace, missing, install, commit, quiet, yes, cache, outdatedDependencies } = {} ) {
        cache ||= {};
        ...
        for ( const name in outdatedDependencies ) {
            ...
            for ( const spec of specs ) {
                ...
                ids[ id ].dependent.add( spec.dependent );
            ...
            specs = Object.values( ids ).map( spec => {
            ...
            for ( const spec of specs ) {
                ...
                // exclude not-updatable dependency
                if ( updatable ) {
                    if ( spec.updatable ) {
                        include = true;
                    }
                ...
                // exclude not-outdated dependency
                if ( outdated ) {
                    if ( spec.outdated ) {
                        include = true;
                    }
                ...
                if ( workspace != null ) {
                    ...
                    if ( !cache.workspacePackages ) {
                        cache.workspacePackages = new Set();

                        const res = findWorkspacePackages();
                        if ( !res.ok ) return res;

                        for ( const pkg of res.data ) {
                            cache.workspacePackages.add( pkg.name );
                        }
                    }

                    if ( workspace ) {
                        if ( !cache.workspacePackages.has( name ) ) continue;
                    }
                    else {
                        if ( cache.workspacePackages.has( name ) ) continue;
                    }
    ...
    async setRepositoryArchived ( archived ) {
        ...
        const res = await gitHubApi.updateRepository( upstream.repoId, {
    ...
    async configureUpstreamRepository () {
        ...
        const gitHubApi = new GitHubApi( process.env.GITHUB_TOKEN ),
            ...
            homepage = ( this.docs.isExists && upstream.docsUrl ) || upstream.homeUrl;
    ...
    async #updateMetadata ( { homepage, author, license, dependabot, commit } = {} ) {
        var res, updated;
        ...
        for ( const pkg of packages ) {
            ...
            if ( !config.homepage || homepage ) {
                const homepage = ( this.docs.isExists && upstream.docsUrl ) || upstream.homeUrl;
