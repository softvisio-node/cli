    ...
    async publishNpm ( latestTag, nextTag ) {
        ...
        res = await repeatAction( async () => {
            const tmpDir = new TmpDir(),
                args = [ "pack", "--json", "--pack-destination", tmpDir.path, this.root ];

            console.log( "Packing:", "npm", args.join( " " ) );

            const res = childProcess.spawnSync( "npm", args, {
                "shell": true,
                "stdio": "pipe",
            } );
            ...
            await stream.promises.pipeline(
                ...
                new tar.TarParserStream(),
                new tar.TarPackStream( {
                    "gzip": true,
                    "onWriteEntry": writeEntry => {
                        if ( executablesPatterns?.test( writeEntry.path ) ) {
