...
import { confirm } from "@softvisio/core/utils";
    ...
    hasPreReleaseDepth () {
        ...
        for ( const name of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"] ) {
            ...
            for ( const dependencyName in config[name] ) {
                ...
                try {
                    if ( dependencyRange.isPreRelease.length ) return result( [500, `Package "${this.name}" has pre-release dependency "${dependencyName}${config[name][dependencyName]}"`] );
                }
                catch ( e ) {
