...
export default class Package {
    ...
    async updateMetadata ( { force } = {} ) {
        ...
        const upstream = await this.git.getUpstream(),
            packages = [this, ...this.packages];

        for ( const pkg of packages ) {
            const config = readConfig( pkg.root + "/package.json" );

            // homepage
            // if ( !config.homepage || force ) config.homepage = upstream.homeUrl;
            if ( !config.homepage || force ) config.homepage = upstream.docsUrl;
            ...
            // license
            if ( !config.license || force ) config.license = process.env.META_LICENSE;
