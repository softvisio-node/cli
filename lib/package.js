...
export default class Package {
    ...
    async getOutdatedDependencies () {
        ...
        try {
            const dependencies = JSON.parse( res.stdout );

            return result( 200, dependencies );
        }
        catch {
            return result( 500 );
        }
    ...
    async checkOutdatedDependencies ( { update, updateMissing, commit, yes, outdatedDependencies } = {} ) {
        var res;

        if ( !outdatedDependencies ) {
            res = await this.getOutdatedDependencies();
