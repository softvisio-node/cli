...
export default class GetText {
        ...
        try {
            ...
            if ( extname === ".vue" ) {
                ...
                if ( sfc.descriptor?.template?.content ) {
                    ...
                    this.#parse( poFile, `<template>${sfc.descriptor.template.content}</template>`, parseVueTemplate, vueTemplateParserOptions, this.#relativePath, start );
                }
                ...
                if ( sfc.descriptor?.script?.content ) {
                    ...
                    this.#parse( poFile, sfc.descriptor.script.content, parseEspree, jsParserOpions, this.#relativePath, start );
                }
            }

            // .js, .mjs, .cjs
            else if ( extname === ".js" || extname === ".mjs" || extname === ".cjs" ) {
                this.#parse( poFile, content, parseEspree, jsParserOpions, this.#relativePath );
            }
            ...
            if ( e instanceof result.Result ) {
                ...
                res = this.#error( e, { "file": this.#absolutePath } );
            }
    ...
    #parseYaml ( poFile, content, file ) {
        ...
        const ejs = template => {
            this.#parseEjs( poFile, template, file );
        };
    ...
    // XXX node
    #parseEjs ( poFile, content, file, node ) {
        const template = new Template( content );

        template.compile();

        return this.#parse( poFile, template.source, parseEspree, ejsParserOpions, file );
    }
    ...
    #parseCallExpression ( ast, node, poFile, file, start ) {
        ...
        if ( method === "ejs" ) {
            if ( node.arguments[0].type === "Literal" ) {
                this.#parseEjs( poFile, node.arguments[0].value, file, node.arguments[0] );
            }
