...
export default class GetText {
        ...
        try {
            ...
            if ( extname === ".vue" ) {
                ...
                if ( sfc.descriptor?.template?.content ) {
                    ...
                    this.#parse( `<template>${sfc.descriptor.template.content}</template>`, parseVueTemplate, vueTemplateParserOptions, start );
                }
                ...
                if ( sfc.descriptor?.script?.content ) {
                    ...
                    this.#parse( sfc.descriptor.script.content, parseEspree, jsParserOpions, start );
                }
            }
        ...
        estraverse.traverse( ast, {
            "enter": ( node, parent ) => {
                if ( node.type === "VAttribute" ) {
                    ...
                    this.#parseCallExpression( ast, node, start );
                }
            },
            "keys": {
    ...
    #parseEjs ( content, start ) {
        ...
        return this.#parse( template.source, parseEspree, ejsParserOpions, start );
    }
    ...
    #parseCallExpression ( ast, node, start ) {
        ...
        if ( method === "ejs" ) {
            ...
            if ( template ) {
                this.#parseEjs( template, arg.loc.start );
            }
