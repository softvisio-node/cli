...
export default class GetText {
    ...
    #parseCallExpression ( ast, node, poFile, file, start ) {
        ...
        // oprions
        if ( node.arguments[1] ) {
            const options = this.#parseOptions( node.arguments[1], { file, start } );

            if ( options.plural ) {
                message.plugalId = options.plural;
    ...
    #parseOptions ( node, location ) {
        ...
        for ( const property of node.properties ) {
            ...
            // plural
            if ( key === "plural" ) {
                const plural = this.#parseMessage( property.value, location );

                if ( plural.value ) {
                    res.plural = plural.value;
    ...
    #parseYaml ( poFile, content, file ) {
        ...
        yaml.parse( content, { "all": true, locale } );
    }
}
