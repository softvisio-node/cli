...
export default class GetText {
        ...
        try {
            ...
            if ( extname === ".vue" ) {
                ...
                if ( sfc.descriptor?.template?.content ) {
                    ...
                    this.#parse( poFile, `<template>${sfc.descriptor.template.content}</template>`, parseVueTemplate, vueTemplateParserOptions, start );
                }
                ...
                if ( sfc.descriptor?.script?.content ) {
                    ...
                    this.#parse( poFile, sfc.descriptor.script.content, parseEspree, jsParserOpions, start );
                }
            }

            // .js, .mjs, .cjs
            else if ( extname === ".js" || extname === ".mjs" || extname === ".cjs" ) {
                this.#parse( poFile, content, parseEspree, jsParserOpions );
            }

            // .yaml
            else {
                this.#parseYaml( poFile, content, this.#relativePath );
            }
        }
        catch ( e ) {
            let res;
    ...
    #parse ( poFile, content, parser, options, start ) {
        ...
        estraverse.traverse( ast, {
            "enter": ( node, parent ) => {
                if ( node.type === "VAttribute" ) {
                    ...
                    this.#parseCallExpression( ast, node, poFile, start );
                }
            },
            "keys": {
    ...
    // XXX node
    #parseEjs ( poFile, content, node ) {
        const template = new Template( content );

        template.compile();

        return this.#parse( poFile, template.source, parseEspree, ejsParserOpions );
    }

    #parseCallExpression ( ast, node, poFile, start ) {
        let method;
        ...
        if ( method === "ejs" ) {
            if ( node.arguments[0].type === "Literal" ) {
                this.#parseEjs( poFile, node.arguments[0].value, node.arguments[0] );
            }
        ...
        // message id
        const msgId = this.#parseMessage( node.arguments[0], { start } );
        ...
        // plural
        if ( node.arguments[1] ) {
            const plural = this.#parseMessage( node.arguments[1], { start } );
