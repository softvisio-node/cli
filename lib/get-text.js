...
export default class GetText {
    ...
    #parseCallExpression ( ast, node, poFile, file, start ) {
        ...
        // ejs
        if ( method === "ejs" ) {
            const template = this.#parseMessage( node.arguments[0], { file, start } );

            if ( !template.value ) return;

            this.#parseEjs( poFile, template.value, file );
