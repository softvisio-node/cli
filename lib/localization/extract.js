    ...
    async extract ( file ) {
        ...
        if ( extname === ".vue" ) {
            ...
            if ( sfc.descriptor?.template?.content ) {
                ...
                const res = this.#parse( messages, `<template>${sfc.descriptor.template.content}</template>`, parseVueTemplate, vueTemplateParserOptions, file, start );
            ...
            if ( sfc.descriptor?.script?.content ) {
                ...
                const res = this.#parse( messages, sfc.descriptor.script.content, parseEspree, jsParserOpions, file, start );
                if ( !res.ok ) return res;
            }
        }
        ...
        // .js, .mjs, .cjs
            ...
            const res = this.#parse( messages, content, parseEspree, jsParserOpions, file );
            if ( !res.ok ) return res;
        }
        ...
        return result( 200, new PoFile( { messages } ) );
    }
    ...
    #parseCallExpressionArgument ( node, location ) {
        ...
        if ( node.type === "Literal" ) {
            ...
            if ( node.quasis.length === 1 ) {
                ...
                throw `Template literal with arguments must be tagged with the "msgid" tag`;
            }
        }
            ...
            if ( node.tag.name === "msgid" ) {
                ...
                throw `Tagged template literal must be tagged with the "msgid" tag`;
            }
        }
