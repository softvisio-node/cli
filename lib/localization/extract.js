...
export default class {
    extract ( file ) {
        ...
        // XXX
        if ( !EXTNAMES.has( extname ) ) return this.#error( `File type ${file} not supported` );

        if ( !fs.existsSync( file ) ) return this.#error( `File ${file} not found` );
    ...
    #parseCallExpressionArgument ( node, location ) {
        ...
        if ( node.type === "Literal" ) {
            ...
            if ( node.quasis.length === 1 ) {
                ...
                return this.#error( `Template literal with arguments must be tagged with the "msgid" tag`, location );
            }
        }
            ...
            if ( node.tag.name === "msgid" ) {
                ...
                return this.#error( `Tagged template literal must be tagged with the "msgid" tag`, location );
            }
        }
