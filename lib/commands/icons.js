...
export default class extends Command {
    ...
    async run () {
        const workPackage = this._findNearestPackage();

        if ( !workPackage ) this._throwError( `Unable to find package` );

        const fileTree = new FileTree(),
            resources = workPackage.root + "/resources";
        ...
        for ( const os in icons ) {
            ...
            for ( const spec of icons[os] ) {
                ...
                for ( const [name, target] of Object.entries( spec.targets ) ) {
                    ...
                    const icon = img.clone().resize( target.w, target.h, jimp.RESIZE_BICUBIC );

                    if ( target.greyscale ) icon.greyscale();

                    fileTree.add( { "path": "resources/" + name, "content": await icon.getBufferAsync( jimp.MIME_PNG ) } );
                }
            }
        }

        return fileTree.write( workPackage.root );
    }
}
