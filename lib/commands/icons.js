...
export default class extends Command {
    static cli () {
        return {
            ...
            "arguments": {
                "icon": {
                    "description": "original icon, should be 1024 x 1024 .png in good quality",
                    "required": true,
                    "schema": { "type": "string" },
                },
            },
    ...
    async run () {
        ...
        for ( const os in icons ) {
            ...
            for ( const name in icons[os] ) {
                ...
                fileTree.add( { "path": name, "content": await icon.getBufferAsync( jimp.MIME_PNG ) } );
            }
        }
        ...
        if ( rootPackage ) {
            return fileTree.write( rootPackage.root + "/resources" );
        }
        else {
            return fileTree.write( "." );
        }
    }
}
