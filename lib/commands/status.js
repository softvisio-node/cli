...
Is dirty:                ${ status.isDirty
    ? ansi.error( " DIRTY " )
    : ansi.ok( " COMMITED " ) }
Branch's ahead/behind:   ${ Object.keys( status.branchStatus )
    .map( branch => branch + ":" + ( status.branchStatus[ branch ].ahead || status.branchStatus[ branch ].behind
        ? ansi.error( " " + status.branchStatus[ branch ].ahead + "/" + status.branchStatus[ branch ].behind + " " )
        : ansi.ok( " OK " ) ) )
    .join( ", " ) }
