...
export default class extends Command {
    ...
    async run () {
        ...
        const { "default": ansi } = await import( "@softvisio/core/ansi" ),
            git = rootPackage.git;
...
Current Release:    ${ansi.hl( id.data.currentRelease || "-" )}
...
Not Pushed Changes: ${Object.keys( id.data.pushStatus )
        ...
        .join( ", " )}
Unreleased Changes: ${id.data.currentReleaseDistance ? ( id.data.currentRelease ? ansi.error( " " + id.data.currentReleaseDistance + " " ) : id.data.currentReleaseDistance ) : ansi.ok( " RELEASED " )}
        `.trim() );
