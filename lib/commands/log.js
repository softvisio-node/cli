...
export default class extends Command {
    ...
    async run () {
        ...
        const previousReleaseVersion = res.data.previousVersion,
            releaseVerion = res.data.version;

        // get changes
        res = await git.getChanges( [ previousReleaseVersion, releaseVerion
            ? releaseVerion.versionString + "~1"
            : null ] );
        if ( !res.ok ) return res;
        ...
        // create changes list
            ...
            if ( previousReleaseVersion && releaseVerion ) {
                console.log( `### Changes between the releases: ${ previousReleaseVersion.versionString }..${ releaseVerion.versionString }` );
            }
            else if ( !previousReleaseVersion && releaseVerion ) {
                console.log( `### Changes for the release: ${ releaseVerion.versionString }` );
            }
            else if ( previousReleaseVersion && !releaseVerion ) {
                console.log( `### Changes since the release: ${ previousReleaseVersion.versionString }` );
            }
            else {
