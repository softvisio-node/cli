...
export default class extends Command {
    ...
    async run () {
        ...
        try {
            var pattern = process.cli.arguments?.pattern ? new RegExp( process.cli.arguments?.pattern, "i" ) : null;
        }
        catch ( e ) {
        ...
        for ( const project of projects ) {
            ...
            if ( !this._isRootPackageDir( workspace + "/" + project ) ) continue;

            const rootPackage = this._findRootPackage( workspace + "/" + project );
            ...
            table.add( {
                "name": ( isPrivate ? ansi.error( " PRV " ) : ansi.ok( " PUB " ) ) + " " + this.#prepareName( project ),
