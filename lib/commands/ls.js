...
export default class extends Command {
    ...
    async #lsWorkspace ( name, path, options ) {
        const fs = await import( "fs" ),
            { "default": Table } = await import( "@softvisio/core/text/table" ),
        ...
        for ( const root of projects ) {
            ...
            table.add( {
                ...
                "latest": await this.#versionFormat( id.data.latestRelease ),
                "current": await this.#versionFormat( id.data.currentRelease ),
    ...
    async #versionFormat ( version ) {
        ...
        return version ? ( semver.minVersion( version ).prerelease.length ? ansi.error( " * " ) + " " + version : version ) : "-";
    }
}
