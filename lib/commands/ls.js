...
export default class extends Command {
    ...
    static cli () {
        return {
            ...
            "arguments": {
                "pattern": {
                    "description": `Filter packages using patterns. Use "." to show all projects.`,
    ...
    async run () {
        ...
        const table = new Table( {
            ...
            "columns": {
                "name": { "title": ansi.hl( "PROJECT" ), "headerAlign": "center", "headerValign": "end" },
        ...
        for ( const pkg of packages ) {
            ...
            table.add( {
                "name": ( isPrivate
                    ? ansi.error( " PRV " )
                    : ansi.ok( " PUB " ) ) + " " + this.#prepareName( pkg ),
    ...
    // private
    #prepareName ( pkg ) {
        const [ owner, name ] = pkg.workspacePath.split( "/" );
