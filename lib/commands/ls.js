...
export default class extends Command {
    ...
    async run () {
        ...
        for ( const pkg of packages ) {
            ...
            const isReleasable = pkg.isReleasable,
            ...
            if ( !res.ok ) return result( [ 500, `Git error: ` + res ] );
            const status = res.data;

            const currentBranchPushStatus = status.pushStatus[ status.head.branch ]?.ahead
                ? `${ status.pushStatus[ status.head.branch ].ahead }`
                : null;

            const isDirty = status.isDirty || currentBranchPushStatus,
                isUnreleased = ( status.currentReleaseDistance || ( !status.head.branch && status.head.abbrev ) ) && pkg.isReleasable;
            ...
            table.add( {
                ...
                "branch": status.head.branch || ( status.head.abbrev
                    ? "HEAD: " + ansi.error( status.head.abbrev )
                    : "" ),
