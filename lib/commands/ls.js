    ...
    async #lsWorkspace ( name, path, options ) {
        ...
        for ( const root of projects ) {
            ...
            const git = rootPackage.git,
                state = ( await git.getState() ).data,
                currentBranchPushStatus = state.pushStatus[state.branch];

            const isDirty = state.isDirty || currentBranchPushStatus,
                isUnreleased = ( !state.currentVersion.isNull && state.currentVersionDistance ) || ( !state.branch && state.abbrev );
            ...
            table.add( {
                ...
                "branch": state.branch || ( state.abbrev ? "HEAD: " + ansi.error( state.abbrev ) : "" ),
                "dirty": state.isDirty,
                "pushed": currentBranchPushStatus ? ansi.error( " " + currentBranchPushStatus + " " ) : "-",
                "next": this.#versionFormat( state.releases.next ),
                "current": this.#versionFormat( state.currentVersion ),
                "unreleased": state.currentVersionDistance ? ( !state.currentVersion.isNull ? ansi.error( " " + state.currentVersionDistance + " " ) : state.currentVersionDistance ) : "-",
            } );
        }
