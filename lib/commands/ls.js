...
export default class extends Command {
    ...
    static cli () {
        return {
            "options": {
                ...
                "dirty": {
                    "description": `show dirty projects only`,
                    "default": false,
                    "schema": { "type": "boolean" },
                },
                "unreleased": {
    ...
    async run () {
        ...
        if ( process.cli.options.dirty || process.cli.options.unreleased ) {
            options.dirty = false;
            options.unreleased = false;

            if ( process.cli.options.dirty ) options.dirty = true;
    ...
    async #lsWorkspace ( name, path, options ) {
        ...
        for ( const root of projects ) {
            ...
            const isDirty = id.isDirty || currentBranchPushStatus,
                isUnreleased = ( !id.currentVersion.isNull && id.currentVersionDistance ) || ( !id.branch && id.hashShort );
            ...
            table.add( {
                ...
                "next": this.#versionFormat( id.releases.next ),
                "current": this.#versionFormat( id.currentVersion ),
