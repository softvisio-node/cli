...
Ignored directories:
  ...
  - "node_modules" directory is ignored if it is relative to the current directory;`,
            ...
            "arguments": {
                "path": {
                    "summary": `File, directory or glob. You can pass "-" to read files list from stdin. If not specified "lint" config from the nearest "package.json" will be used.`,
                    "maxItems": false,
                    "schema": { "type": "string" },
                },
            },
    ...
    async run () {
        ...
        if ( !process.cli.arguments.path.length ) {
            const root = this._getPackageRoot();
            ...
            process.cli.arguments.path = pkg.lint;
        }

        // read from stdin
        else if ( process.cli.arguments.path.length === 1 && process.cli.arguments.path[0] === "-" ) await this._readStdin();
