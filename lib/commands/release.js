const { throwError, getProjectRoot, getSubProjects } = require( "../util" );
...
module.exports = class {
    ...
    async run () {
        ...
        // check master branch
        if ( !id.branch || id.branch !== "master" ) throwError( `Project is not on "master" branch.` );

        // check for uncommited changes
        if ( id.isDirty ) throwError( `Working copy or sub-repositories has uncommited changes or untracked files.` );

        // check distance from the last release
        if ( id.release && !id.releaseDistance ) throwError( `No changes  since the latesst release.` );
        ...
        if ( !version ) {
            if ( process.cli.options.bugfix ) throwError( `Bugfix is impossible on initial release.` );
        ...
        if ( version === newVersion ) throwError( `Nothing to do.` );

        var releases = await git.getReleases();

        if ( !releases.ok ) throwError( `Unable to get releases. ${releases}` );

        if ( releases.data["v" + newVersion] ) throwError( `Version "${newVersion}" is already released.` );
        ...
        // confirm release
        if ( ( await confirm( "\nContinue release process?", ["y", "n"] ) ) === "n" ) throwError( "Terminated." );
        ...
        // update config.xml
        this._updateConfigXml( root, newVersion );
        ...
        if ( subProjects.length ) {
            for ( const subProject of subProjects ) {
                ...
                // update package.json
                const pkg = require( subProject + "/package.json" );
                pkg.version = newVersion;
                fs.writeFileSync( subProject + "/package.json", JSON.stringify( pkg, null, 4 ) + "\n" );

                // update config.xml
                this._updateConfigXml( subProject, newVersion );
            }
        }
    ...
    async _updateChanges ( version, newVersion, git, root ) {
        ...
        try {
            ...
            throwError( "Unable to compose changelog." );
        }
