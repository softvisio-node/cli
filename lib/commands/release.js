...
module.exports = class {
    ...
    async run () {
        ...
        if ( releases.data[CONST.VERSION_PREFIX + newVersion] ) throwError( `Version "${newVersion}" is already released.` );
        ...
        // confirm release
        if ( ( await confirm( "\nContinue release process?", ["n", "y"] ) ) === "n" ) throwError( "Terminated." );
        ...
        // generate wiki
        // TODO
        // if ( fs.existsSync( root + "./wiki/.git" ) ) {
        ...
        if ( await git.getUpstream() ) {
            ...
            while ( 1 ) {
                ...
                res = await git.run( "push", "origin", "-f", `refs/tags/${CONST.VERSION_PREFIX + newVersion}`, "refs/tags/latest" );
    ...
    async _updateChanges ( version, newVersion, git, root ) {
        ...
        var log = `## ${CONST.VERSION_PREFIX + newVersion} (${new Date().toUTCString()})\n\n`;
