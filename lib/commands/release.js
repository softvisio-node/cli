    ...
    async run () {
        ...
        if ( semver.prerelease( version ) ) {
            ...
            if ( semver.patch( version ) ) {
                if ( process.cli.arguments.release !== "patch" ) this._throwError( `Release version must be "patch".` );
            }
            else if ( semver.minor( version ) ) {
                if ( process.cli.arguments.release !== "minor" ) this._throwError( `Release version must be "minor".` );
            }
            else if ( semver.major( version ) ) {
                if ( process.cli.arguments.release !== "major" ) this._throwError( `Release version must be "major".` );
            }
        ...
        if ( !process.cli.arguments["pre-release-tag"] ) {
            for ( const pkg of [rootPackage, ...linkedWorkspaces] ) {
                if ( !this.#checkPreReleaseDeps( pkg ) ) {
                    this._throwError( `Package "${pkg.name}" has pre-released dependencies. You can issue pre-release version only or update package dependencies to non pre-release versions.` );
                }
            }
        }
        ...
        for ( const workspace of linkedWorkspaces ) {
            if ( !workspace.isPrivate ) await workspace.publish( latestMajorTag, latestTag );
        }
    }
    ...
    #checkPreReleaseDeps ( pkg ) {
        ...
        for ( const name of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"] ) {
            ...
            for ( const dependencyName in config[name] ) {
                ...
                try {
                    if ( semver.minVersion( dependencyVersion ).prerelease.length ) return false;
                }
                catch ( e ) {
                    this._throwError( `Unable to parse dependency version "${dependencyName + "@" + config[name][dependencyName]}"` );
                }
            }
        }
