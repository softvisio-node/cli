...
module.exports = class extends Command {
    ...
    async run () {
        ...
        if ( id.data.releases[CONST.VERSION_PREFIX + newVersion] ) this._throwError( `Version "${newVersion}" is already released.` );

        const latest = version === "0.0.0" || id.data.latestRelease === CONST.VERSION_PREFIX + version ? true : false;
        ...
        if ( await git.getUpstream() ) {
            ...
            while ( 1 ) {
                ...
                const params = ["push", "origin", "-f", `refs/tags/${CONST.VERSION_PREFIX + newVersion}`];

                if ( latest ) params.push( "refs/tags/latest" );
