...
module.exports = class extends Command {
    static cli () {
        return {
            ...
            "arguments": {
                "release": {
                    "summary": `Release type. Allowed values: "major", "minor", "patch".`,
                    "minItems": 1,
                    "schema": { "type": "string", "enum": ["major", "minor", "patch"] },
                },
                "pre-release-tag": {
                    "summary": `Pre-release tag. Allowed values: "alpha", "beta", "rc".`,
                    "schema": { "type": "string", "enum": ["alpha", "beta", "rc"] },
                },
            },
        };
    }
    ...
    async run () {
        ...
        if ( await git.getUpstream() ) {
            while ( 1 ) {
                ...
                res = await git.run( "push", "--atomic", "origin", id.branch, "v" + newVersion, "latest", "--force" );
        ...
        for ( const workspace of linkedWorkspaces ) {
            if ( !workspace.isPrivate ) await workspace.publish();
        }
    }
