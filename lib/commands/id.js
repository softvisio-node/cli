...
Not Pushed Changes: ${Object.keys( pushStatus.data )
        .map( branch => branch + ":" + ( pushStatus.data[branch] ? ansi.error( " " + pushStatus.data[branch] + " " ) : ansi.ok( " PUSHED " ) ) )
        .join( ", " )}
Unreleased Changes: ${id.data.releaseDistance ? ansi.error( " " + id.data.releaseDistance + " " ) : ansi.ok( " RELEASED " )}
        `.trim() );
