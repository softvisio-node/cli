...
export default class extends Command {
    ...
    async #installGlobal () {
        ...
        if ( fs.existsSync( hooksPath.data + "/pre-commit" ) && ( await utils.confirm( "Global pre-commit hook is already exists. Overwrite?", ["n", "y"] ) ) !== "y" ) return;
    ...
    async #installLocal () {
        ...
        if ( fs.existsSync( hooksPath + "/pre-commit" ) && ( await utils.confirm( "Local pre-commit hook is already exists. Overwrite?", ["n", "y"] ) ) !== "y" ) return;
