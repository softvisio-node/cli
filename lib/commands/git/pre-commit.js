...
module.exports = class extends Command {
    ...
    _filterFiles ( root, staged ) {
        ...
        if ( pkg.lint ) rootLint = pkg.lint.map( pattern => new Minimatch( util.preparePattern( pattern, root ), { "dot": true } ) );
        ...
        for ( const subProjectRoot of subProjects ) {
            ...
            if ( pkg.lint ) subProjectsLint[subProjectRoot] = pkg.lint.map( pattern => new Minimatch( util.preparePattern( pattern, root + "/" + subProjectRoot ), { "dot": true } ) );
        }
