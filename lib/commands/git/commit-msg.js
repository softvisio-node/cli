    ...
    async run () {
        ...
        if ( commit.type ) {
            ...
            if ( pkg.cliConfig.commits?.types?.length && !pkg.cliConfig.commits.types.includes( commit.type ) ) {
                this.#throwError( `Commit type is invalid. Allowed types: ${ [ ...pkg.cliConfig.commits.types ].map( item => `"${ item }"` ).join( ", " ) }.` );
            }

            // check scope
            if ( commit.scope ) {
                if ( pkg.cliConfig.commits?.scopes?.length && !pkg.cliConfig.commits.scopes.includes( commit.scope ) ) {
                    this.#throwError( `Commit scope is invalid. Allowed scopes: ${ [ ...pkg.cliConfig.commits.scopes ].map( item => `"${ item }"` ).join( ", " ) }.` );
                }
            }
            else {
                if ( pkg.cliConfig.commits?.scopeRequired ) this.#throwError( `Commit scope is required.` );
            }
        }
        else {
            if ( pkg.cliConfig.commits?.strict ) this.#throwError( `Commit message must match conventional commits.` );
        }
    }
