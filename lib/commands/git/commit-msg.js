...
export default class extends Command {
    ...
    async run () {
        ...
        const msg = fs.readFileSync( process.cli.arguments.path, "utf8" ),
            commit = msg.match( COMMIT_RE );

        if ( commit ) {

            // check type
            if ( pkg.commitsConfig.types.size && !pkg.commitsConfig.types.has( commit.groups.type ) ) {
                this.#throwError( `Commit type is invalid. Allowed types: ${[...pkg.commitsConfig.types].map( item => `"${item}"` ).join( ", " )}.` );
            }

            // check scope
            if ( commit.groups.scope ) {
                if ( pkg.commitsConfig.scopes.size && !pkg.commitsConfig.scopes.has( commit.groups.scope ) ) {
                    this.#throwError( `Commit scope is invalid. Allowed scopes: ${[...pkg.commitsConfig.scopes].map( item => `"${item}"` ).join( ", " )}.` );
                }
            }
            else {
                if ( pkg.commitsConfig.require_scope ) this.#throwError( `Commit scope is required.` );
            }
        }
        else {
    ...
    // private
    #throwError ( msg ) {
        this._throwError( ansi.error( " Error " ) + "\n" + msg + "\nRefer to the documentation: https://softvisio.github.io/cli/#/commits" );
    }
}
