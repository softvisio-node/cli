...
`,
    ...
    #parseCallExpression ( ast, node, messages, file, start ) {
        ...
        // single form
        const arg = this.#parseCallExpressionArgument( node.arguments[0], { file, start } );
        if ( !arg.string ) return;

        message.msgId = arg.string;
        if ( arg.isTemplate ) message.isTemplate = true;

        // plural form
        if ( node.arguments[1] ) {
            const arg = this.#parseCallExpressionArgument( node.arguments[1], { file, start } );
            if ( !arg.string ) return;

            message.msgIdPlural = arg.string;
            if ( arg.isTemplate ) message.isTemplate = true;
        }
        ...
        // plural form
        if ( message.msgIdPlural ) messages[message.msgId].msgIdPlural = message.msgIdPlural;

        // flags
        if ( message.isTemplate ) messages[message.msgId].flags = ["javascript-format"];

        // references
        if ( file ) {
            messages[message.msgId].references ||= [];

            const reference = `${file}:${node.loc.start.line}:${node.loc.start.column}`;

            messages[message.msgId].references.push( reference );
        }

        // extracted comments
        if ( extractedComments.length ) {
            if ( messages[message.msgId].extractedComments ) {
                messages[message.msgId].extractedComments += "\n" + extractedComments.join( "\n" );
            }
            else {
                messages[message.msgId].extractedComments = extractedComments.join( "\n" );
            }
        }
    }
    ...
    // protected
    _throwError ( message, { file, loc, start } = {} ) {
        if ( file ) {
            if ( loc ) {
                if ( start ) {
                    if ( loc.line === 1 ) loc.column += start.column;
                    loc.line += start.line - 1;
                }
