...
const vueTemplateParserOptions = { "sourceType": "module" },
    esmParserOpions = { "sourceType": "module", "ecmaVersion": "latest", "loc": true, "comment": true },
    cjsParserOpions = { "sourcetype": "commonjs", "loc": true, "comment": true };
...
`,
        ...
        for ( const file of process.cli.arguments.files ) {
            ...
            if ( file.endsWith( ".vue" ) ) {
                ...
                if ( sfc.descriptor?.template?.content ) {
                    this.#parse( messages, `<template>\n${sfc.descriptor.template.content}\n</template>`, parseVueTemplate, vueTemplateParserOptions, file );
                }

                // vue script
                if ( sfc.descriptor?.script?.content ) {
                    this.#parse( messages, sfc.descriptor.script.content, parseEspree, esmParserOpions, file );
                }
            }

            // .cjs
            else if ( file.endsWith( ".cjs" ) ) {
                this.#parse( messages, content, parseEspree, cjsParserOpions, file );
            }

            // .js, .mjs
            else if ( file.endsWith( ".js" ) || file.endsWith( ".mjs" ) ) {
                this.#parse( messages, content, parseEspree, esmParserOpions, file );
            }
        }
        ...
        estraverse.traverse( ast, {
            "enter": ( node, parent ) => {
                if ( node.type === "VAttribute" ) {
                    ...
                    this.#parseCallExpression( ast, node, messages, file );
                }
            },
            "keys": {
    ...
    #parseCallExpression ( ast, node, messages, file ) {
        let method;
        ...
        // extract comments
        const comments = ast.comments || ast.templateBody?.comments;
