...
import { parseYamlConfig } from "#core/config";
...
`,
    ...
    #parseYaml ( messages, content, file ) {
        const schema = yaml.DEFAULT_SCHEMA.extend( [
            new yaml.Type( "!i18nt", {
                ...
                construct ( data ) {
                    ...
                    messages[data[0]].pluralId ??= data[1];

                    if ( !messages[data[0]].references.includes( file ) ) {
                        messages[data[0]].references.push( file );
                    }
        ...
        parseYamlConfig( content, { "all": true, schema } );
    }
}
