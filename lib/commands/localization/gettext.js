...
`,
    ...
    #parseCallExpression ( ast, node, messages, file, start ) {
        ...
        if ( method !== "i18n" && method !== "i18nd" ) return;
        ...
        // single form
        const arg = this.#parseCallExpressionArgument( method === "i18n" ? node.arguments[0] : node.arguments[1], { file, start } );
        ...
        // plural form
        if ( node.arguments[1] ) {
            const arg = this.#parseCallExpressionArgument( method === "i18n" ? node.arguments[1] : node.arguments[2], { file, start } );
            if ( !arg.string ) return;
