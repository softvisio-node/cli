...
`,
        ...
        for ( const file of process.cli.arguments.files ) {
            ...
            if ( file.endsWith( ".vue" ) ) {
                ...
                if ( sfc.descriptor?.script?.content ) {
                    this.#parse( messages.sfc.descriptor.script.content, parseEspree, { "sourceType": "module", "ecmaVersion": 2015 }, file );
                }
            }

            // .cjs
            else if ( file.endsWith( ".cjs" ) ) {
                this.#parse( messages, content, parseEspree, { "sourceType": "commonjs" }, file );
            }

            // .js, .mjs
            else if ( file.endsWith( ".js" ) || file.endsWith( ".mjs" ) ) {
                this.#parse( messages, content, parseEspree, { "sourceType": "module", "ecmaVersion": 2015 }, file );
            }
        }
    ...
    #parseCallExpression ( node, messages, file ) {
        ...
        // extract comments
        for ( const node of node.arguments ) {
            if ( node.leadingComments ) {
                extractedComments.push( node.leadingComments
            ...
            if ( node.trailingComments ) {
                extractedComments.push( node.trailingComments
                    .map( block => block.value.trim() )
                    .filter( value => value )
                    .join( " " ) );
            }
