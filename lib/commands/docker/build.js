...
export default class extends Command {
    ...
    async run () {
        ...
        if ( !state.ok ) this._throwError( state, dockerRoot );
        if ( !state.data.hash ) this._throwError( `Unable to identify current changeset.`, dockerRoot );
        state = state.data;

        // define tag to clone
        const tag = process.cli.arguments.tag || dockerConfig["x-build"].default_tag || state.branch;

        const clone = state.isDirty || !( state.branch === tag || state.hash.startsWith( tag ) || state.tags.includes( tag ) );
        ...
        if ( dockerConfig["x-build"].auto_tags ) {
            ...
            for ( const tag of dockerConfig["x-build"].auto_tags ) {
                if ( state.branch === tag || state.tags.includes( tag ) ) tags.add( tag );
            }
        }
        ...
        for ( const service of services ) {
            try {
                ...
                labels["git.branch"] = state.branch || "";
                labels["git.hash"] = state.hash;
                labels["git.date"] = state.date;
                labels["git.tags"] = state.tags.sort().join( "," );
