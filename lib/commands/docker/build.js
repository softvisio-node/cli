...
module.exports = class extends Command {
    ...
    async run () {
        ...
        let tag = process.cli.arguments.tag || dockerConfig["x-build_default_tag"] || id.data.branch;
        ...
        // apply tags mapping
        if ( dockerConfig["x-build_tags_mapping"] && dockerConfig["x-build_tags_mapping"][tag] ) tag = dockerConfig["x-build_tags_mapping"][tag];

        const tags = { [tag]: true };

        // apply auto tags
        if ( dockerConfig["x-build_auto_tags"] ) {
            if ( !Array.isArray( dockerConfig["x-build_auto_tags"] ) ) dockerConfig["x-build_auto_tags"] = [dockerConfig["x-build_auto_tags"]];

            const possibleTags = {};
            if ( id.data.branch ) possibleTags[id.data.branch] = true;
            id.data.tags.forEach( tag => ( possibleTags[tag] = true ) );

            for ( const tag of dockerConfig["x-build_auto_tags"] ) {
        ...
        for ( const service of Object.values( dockerConfig.services ) ) {
            ...
            for ( const tag in tags ) {
                const additionalImages = service.build["x-images"] ? ( Array.isArray( service.build["x-images"] ) ? service.build["x-images"] : [service.build["x-images"]] ) : [];
