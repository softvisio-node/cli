...
module.exports = class extends Command {
    static cli () {
        return {
            ...
            "arguments": {
                "tag": {
                    "summary": "Git branch, tag or hash.",
                    "minItems": 1,
                    "maxItems": 1,
                    "schema": { "type": "string" },
                },
    ...
    async run () {
        ...
        const clone = !id.data.isDirty && ( id.data.branch === process.cli.arguments.tag || id.data.hash.indexOf( process.cli.arguments.tag ) === 0 || Object.fromEntries( id.data.tags.map( tag => [tag, true] ) )[process.cli.arguments.tag] ) ? false : true;
        ...
        if ( clone ) {
            ...
            process.stdout.write( `Checking out "${process.cli.arguments.tag}" ... ` );
            res = await git.run( "checkout", process.cli.arguments.tag );
            if ( !res.ok ) this._throwError( res );
            console.log( res + "\n" );
        }
        ...
        // main registry
        images.push( `${dockerConfig.registry}:${process.cli.arguments.tag}` );

        // mirrors
        if ( dockerConfig.mirrors ) {
            for ( const mirror of dockerConfig.mirrors ) {
                images.push( `${mirror}:${process.cli.arguments.tag}` );
            }
        }
