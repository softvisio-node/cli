...
module.exports = class extends Command {
    static cli () {
        return {
            "summary": "Run vim rpc service.",
            "description": "Start vim RPC server on address 127.0.0.1:55555.",
        };
    }
    ...
    async run () {
        ...
        var server = net.createServer( socket => {
            ...
            rl.on( "line", async data => {
                ...
                const res = this["CMD_" + data[1].cmd]( data[1] );
...
${data.data}
    ...
    CMD_src ( data ) {
        ...
        if ( data.path == null ) data.path = "temp";

        if ( data.ft ) {
            if ( data.ft === "sh" ) {
                if ( path.extname( data.path ).toLowerCase() !== ".sh" ) data.path += ".sh";
            }
        }

        return new File( data.path, { "data": data.data } ).run( data.action );
    }
};
