...
export default class Git {
    ...
    async getUpstream ( url ) {
        const { "default": Upstream } = await import( "./git/upstream.js" );
    ...
    async getId () {
        ...
        this.run( "log", "-1", "--pretty=format:%H%n%cI%n%D" ).then( res => {
            if ( !res.ok ) {
                if ( res.reason.indexOf( "does not have any commits yet" ) === -1 ) {
                    hasError = true;
                }
            }
            else {
        ...
        this.run( "describe", "--tags", "--always", "--match", "v[[:digit:]]*" ).then( res => {
            if ( !res.ok ) {
                if ( res.reason.indexOf( "Not a valid object name HEAD" ) === -1 ) {
                    hasError = true;
                }
                ...
                if ( match && semver.valid( match[1] ) ) {
                    id.currentRelease = match[1];

                    id.currentReleaseDistance = +match[2];

                    cv.end();
                }
                else if ( semver.valid( res.data ) ) {
                    id.currentRelease = res.data;

                    id.currentReleaseDistance = 0;
                ...
                // release distance from the previous version tag wasn't found
                    ...
                    this.run( "rev-list", "HEAD", "--count" ).then( res => {
                        if ( !res.ok ) {
                            ...
                            id.currentReleaseDistance = +res.data.trim();
                        }
