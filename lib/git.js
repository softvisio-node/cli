...
import Upstream from "#lib/git/upstream";
...
export default class Git {
    ...
    async run ( ...args ) {
        ...
        return new Promise( resolve => {
            child_process.execFile( "git", args, { "encoding": "utf8", "stdio": ["pipe", "pipe", "pipe"], "maxBuffer": GIT_MAX_BUFFER_SIZE }, ( err, stdout, stderr ) => {
                if ( err ) {
    ...
    async getChanges ( tag ) {
        ...
        if ( res.data ) {
            for ( const commit of res.data.split( "\n" ) ) {
                ...
                commits.push( {
                    "hash": commit.substr( 0, idx ),
                    "message": commit.substring( idx + 1 ),
                } );
            }
        }
    ...
    async getId () {
        ...
        this.run( "log", "-1", "--pretty=format:%H%n%cI%n%D" ).then( res => {
            if ( !res.ok ) {
                ...
                id.hashShort = id.hash.substr( 0, Changes.HASH_SHORT_LENGTH );
        ...
        this.run( "branch", "-v", "--no-color" ).then( res => {
            if ( !res.ok ) {
                ...
                for ( const line of res.data.split( /\n/ ) ) {
                    ...
                    if ( match ) {
                        id.pushStatus[match.groups.branch || match.groups.head] = match.groups.ahead ? +match.groups.ahead : 0;
                    }
                    else {
        ...
        this.run( "tag" ).then( res => {
            if ( !res.ok ) {
                ...
                id.releases = new Releases( res.data.split( /\n/ ) );
            }
