...
export default class {
    ...
    // public
    async build ( packageName, codenames ) {
        var packages;
    ...
    async update () {
        ...
        for ( const codename of this.#getCodenames().data ) {
            ...
            fs.mkdirSync( this.root + `/dists/${ codename }/${ this.config.component }/binary-amd64`, {
                "recursive": true,
            } );
            ...
            res = this.#spawnSync(
                ...
                [
                    ...
                    "--arch=amd64",
                    "packages",
                    `dists/${ codename }/${ this.config.component }/binary-amd64`,
                ],
                {
                    "stdio": "pipe",
                }
            ...
            fs.writeFileSync( this.root + `/dists/${ codename }/${ this.config.component }/binary-amd64/Packages`, res.data.stdout );
    ...
    #deleteOldPackages () {
        ...
        for ( const file of files ) {
            const res = this.#getDistInfo( this.root + "/dists/" + file );
            if ( !res.ok ) return res;
        ...
        for ( let dists of Object.values( packages ) ) {
            ...
            for ( const dist of dists ) {
                console.log( "Remove dist:", dist.file );
    ...
    #getDistInfo ( file ) {
        ...
        res = this.#spawnSync( "ar", [ "p", file, "control.tar.gz" ], { "stdio": "pipe" } );
        if ( !res.ok ) return res;
