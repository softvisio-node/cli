...
export default class Deb {
    ...
    async checkRepository () {
        ...
        if ( !( await exists( this.distsRoot ) ) ) {
            ...
            res = await git.run( "ls-remote", "--branches", "--refs", "--quiet", upstream.sshUrl, "dists" );
            if ( !res.ok ) return res;

            // create "dists" branch
            if ( !res.data ) {
                res = await git.run( "init", "--initial-branch", "dists", this.distsRoot );
                if ( !res.ok ) return res;

                res = await git.run( "commit", "--allow-empty", "-m", "chore: init" );
                if ( !res.ok ) return res;

                res = await git.run( "push", "--set-upstream", upstream.sshUrl, "dists" );
                if ( !res.ok ) return res;
            }

            // clone "dists" branch
            else {
                res = await git.run( "clone", "--single-branch", "--branch", "dists", upstream.sshUrl, "dists" );
                if ( !res.ok ) return res;
            }
        }
