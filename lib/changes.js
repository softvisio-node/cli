...
export default class Changes {
    ...
    get breaking () {
        this.#breaking ??= this.#changes.filter( commit => commit.breaking ).sort( this.#sort );

        return this.#breaking;
    }

    get feat () {
        this.#feat ??= this.#changes.filter( commit => commit.type === "feat" ).sort( this.#sort );

        return this.#feat;
    }

    get fix () {
        this.#fix ??= this.#changes.filter( commit => commit.type === "fix" ).sort( this.#sort );
    ...
    // private
    #sort ( a, b ) {
        return a.type.localeCompare( b.type ) || a.scope.localeCompare( b.scope ) || a.description.localeCompare( b.description );
    }
}
