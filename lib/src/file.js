...
const TYPE = {
    "js": { "parser": "babel", "yamlComments": true, "eslint": true, "terser": true },
    "vue": { "parser": "vue", "yamlComments": true, "eslint": true },
    "yaml": { "parser": "yaml" },
    "css": { "parser": "css" },
    "scss": { "parser": "scss" },
    "less": { "parser": "less" },
...
module.exports = class {
    ...
    constructor ( filePath, options = {} ) {
        ...
        this.#type = TYPE[EXT[path.extname( this.#path )]];
    }

    run ( action ) {
        if ( this.#ignoreUnsupported && !this.#type ) return result( [202, "File ignored"] );
        ...
        if ( md5 !== this.#md5 ) {
            ...
            if ( !this.#noSave ) fs.writeFileSync( this.#path, this.#data );
        }
        else {
