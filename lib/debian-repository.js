...
export default class DebianRepository {
    ...
    async update ( { deleteOutdatedPackages } = {} ) {
        ...
        res = await this.git.exec( [ "commit", "--amend", "-m", "chore: update distributions" ] );
        if ( !res.ok ) return res;
    ...
    #deleteOutdatedPackages () {
        ...
        for ( let dists of Object.values( packages ) ) {
            ...
            dists = dists.sort( ( a, b ) => {
                return a.epoch - b.epoch || new SemanticVersion( a.version ).compare( b.version ) || a.revision - b.revision;
            } );
